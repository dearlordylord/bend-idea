...
/** Uses the pinned compiler source via Bun; the installed older binary is a
  * negative probe.
  */
final class BendCliCheckBackendTest:
  ...
  private def snapshot(
    ...
    BendCheckSnapshot(
      ...
      text,
      4L,
      BendToolchainSelection(executable.toString, base.toString, "", true, 1L)
    )
  ...
  @Test def missingAndUnsupportedCompilerNeverCheckSource(): Unit =
    withCompiler { executable =>
      ...
      val installed =
        Path.of(System.getProperty("user.home"), ".bend", "bin", "bend")
      if Files.isExecutable(installed) then
        val unsupported =
          backend(executable).check(snapshot(installed, source, original))
        assertEquals(BendCheckOutcome.Unavailable, unsupported.outcome)
