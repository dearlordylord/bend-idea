...
final class BendCliVerdictDecoderTest:
  ...
  @Test def checkOnlyCapabilityRequiresItsOwnHelpLine(): Unit =
    ...
    assertTrue(supported.isInstanceOf[BendCliVerdictDecoder.Capability.Supported])
    val quoted = BendCliVerdictDecoder.capability(
      BendProcessOutcome.Exited(0, s"Vendor note: $checkOnlyLine\n"))
    assertTrue(quoted.isInstanceOf[BendCliVerdictDecoder.Capability.Unavailable])
    val unsupported = BendCliVerdictDecoder.capability(
      BendProcessOutcome.Exited(0, "bend <file.bend> runs source only\n"))
    assertTrue(unsupported.isInstanceOf[BendCliVerdictDecoder.Capability.Unavailable])

  @Test def onlyWholeDocumentedReportsEstablishSuccessOrReliance(): Unit =
    val complete = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(0, "All terms check.\n"))
    ...
    val relied = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(0,
    ...
    val pluralReliance = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(0,
    ...
    val inconsistentCount = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(0,
      "All terms check, but 2 defs rely on unsafe or foreign code:\n- main\n"))
    assertUnknown(inconsistentCount)
    val extraOutput = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(0,
      "Warning: partial log\nAll terms check.\n"))
    assertUnknown(extraOutput)

  @Test def onlyCompleteTodoAndCompilerErrorReportsEstablishFailures(): Unit =
    val incomplete = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1,
    ...
    val pluralIncomplete = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1,
      "Error: 2 TODOs found.\nThe code is incomplete, and not a valid proof yet.\n"))
    ...
    val error = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1, compilerError))
    ...
    val changedErrorTail = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1,
      "Error:\n- expected : a defined name\n- observed : missing_name\n" +
    ...
    val changedTodo = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1,
      "Error: 1 TODO found.\nTODOs may be okay in this compiler\n"))
    assertUnknown(changedTodo)
    val unfamiliarError = BendCliVerdictDecoder.check(BendProcessOutcome.Exited(1,
  ...
  @Test def processLimitsAndCancellationCannotCarryVerdictFacts(): Unit =
    val timeout = BendCliVerdictDecoder.check(BendProcessOutcome.TimedOut("All terms check."))
    ...
    assertTrue(BendCliVerdictDecoder.capability(BendProcessOutcome.TimedOut("")).
      isInstanceOf[BendCliVerdictDecoder.Capability.TimedOut])
    assertTrue(BendCliVerdictDecoder.capability(BendProcessOutcome.Canceled("")).
      isInstanceOf[BendCliVerdictDecoder.Capability.Canceled])
    assertTrue(BendCliVerdictDecoder.capability(BendProcessOutcome.OutputLimit("")).
