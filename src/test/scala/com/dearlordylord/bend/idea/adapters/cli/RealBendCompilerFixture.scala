...
/** Test-only access to the compiler and runtime explicitly supplied to the real
  * compiler fixtures.
  */
object RealBendCompilerFixture:
  final case class Inputs(
      compilerDirectory: Path,
      bunExecutable: Path,
      main: Path,
      base: Path,
      bendCommit: String,
    ...
    def writeLauncher(path: Path, beforeSource: String = ""): Path =
      ...
      require(
        ...
        s"Could not mark Bend test launcher executable: $path"
      )
      path
  ...
  private def loadInputs(): Inputs =
    ...
    val gitRoot = process(
      List(
        "git",
        "-C",
        compilerDirectory.toString,
        "rev-parse",
        "--show-toplevel"
      )
    )
    require(
      Path.of(gitRoot).toRealPath() == compilerDirectory,
      s"BEND_TEST_COMPILER_DIR must name the Bend checkout root; observed $gitRoot"
    )
    val bendCommit = process(
      List(
        "git",
        "-C",
        compilerDirectory.toString,
    ...
    require(
      changed.isEmpty,
      s"Bend checkout has tracked or untracked changes: $compilerDirectory\n$changed"
    )
    ...
    require(
      Files.isExecutable(bunExecutable),
      s"BEND_TEST_BUN is not executable: $bunExecutable"
    )
    ...
    System.out.println(
      s"Real Bend test inputs: commit=$bendCommit checkout=$compilerDirectory " +
    ...
    val value =
      Option(System.getenv(name)).map(_.trim).filter(_.nonEmpty).getOrElse {
        throw new IllegalStateException(
          s"$name is required by real Bend tests; provision the pinned inputs described in README.md"
        )
      }
  ...
  private def requiredProperty(name: String): String =
    Option(System.getProperty(name)).map(_.trim).filter(_.nonEmpty).getOrElse {
      throw new IllegalStateException(
        s"Missing Gradle test pin '$name'; run the real Bend checks through ./gradlew"
      )
    }
  ...
  private def process(
    ...
    try
      ...
      if !process.waitFor(15, TimeUnit.SECONDS) then
        ...
        throw new IllegalStateException(
          s"Timed out verifying real Bend test input: ${command.mkString(" ")}"
        )
