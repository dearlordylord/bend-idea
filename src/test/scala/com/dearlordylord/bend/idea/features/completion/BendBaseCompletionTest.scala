...
final class BendBaseCompletionTest extends BasePlatformTestCase:
  ...
  def testAlternateBaseSourceAndSignatureComments(): Unit =
    val first = base("first.bend", "# First implementation\ndef Nat.plus(x: Nat) -> Nat:\n  x\ndef Nat.please():\n  0\n")
    val second = base("second.bend", "# Alternate source\ndef Nat.plus(y: U32) -> U32:\n  y\ndef Nat.please():\n  0\n")
    select(first)
    val one = items("import Base\ndef main():\n  Nat.pl<caret>").find(_.getLookupString == "plus").get
  ...
  def testSourceUpdateInvalidatesCandidatesAndSignature(): Unit =
    val path = base("updated.bend", "def Nat.old(x: Nat) -> Nat:\n  x\ndef Nat.older():\n  0\n")
    select(path)
    assertTrue(items("import Base\ndef main():\n  Nat.ol<caret>").exists(_.getLookupString == "old"))
    Files.writeString(path, "def Nat.new(y: U32) -> U32:\n  y\ndef Nat.newer():\n  0\n")
  ...
  def testCatalogIsBoundedAndUnsavedBaseDocumentWins(): Unit =
    ...
    def snapshot = sourceService.base(path.toString, settings.selection.configurationRevision)
      .asInstanceOf[BendBaseState.Available].source
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = document.setText("def Nat.second(x: U32) -> U32:\n  x\n"))
    assertTrue(items("import Base\ndef main():\n  Nat.se<caret>").exists(_.getLookupString == "second") ||
  ...
  def testMissingBaseKeepsCurrentFileNamesAndShowsSetup(): Unit =
    ...
    assertEquals(BendBaseState.Missing(missing.toString),
      getProject.getService(classOf[BendLibrarySource]).base(settings.selection.baseSource,
  ...
  def testDottedCompletionReplacesExistingSuffixOnly(): Unit =
    ...
    val found = items("import Base\ndef main():\n  Nat.ad<caret>d.extra").find(_.getLookupString == "add").get
    myFixture.getLookup.setCurrentItem(found)
    myFixture.finishLookup('\t')
    assertTrue(myFixture.getEditor.getDocument.getText.endsWith("  Nat.add.extra"))

  def testDeclarationHeaderDoesNotOfferBaseMembers(): Unit =
    select(base("header.bend", "def Nat.plus():\n  0\n"))
    assertFalse(items("import Base\ndef Nat.pl<caret>").exists(_.getLookupString == "plus"))

  def testCurrentFileWinsByNameAndCategoryWithoutMergingKinds(): Unit =
    select(base("shadow.bend", "type Unit is Data:\n  Unit{}\ndef local():\n  0\n"))
    val matches = items("import Base\ntype Unit is Data:\n  Unit{}\ndef main():\n  Un<caret>")
      .filter(_.getLookupString == "Unit")
    assertEquals(2, matches.length)
    assertEquals(Set("datatype", "constructor"), matches.map(rendered(_).getTypeText).toSet)

  def testBaseIsOnlyOfferedWhenImported(): Unit =
    select(base("visibility.bend", "def Nat.plus():\n  0\ndef Nat.please():\n  0\n"))
    assertFalse(items("def main():\n  Nat.pl<caret>").exists(_.getLookupString == "plus"))
    assertFalse(items("# import Base\ndef main():\n  Nat.pl<caret>").exists(_.getLookupString == "plus"))
    assertTrue(items("import Base # available\ndef main():\n  Nat.pl<caret>").exists(_.getLookupString == "plus"))
    assertFalse(items("def earlier():\n  0\nimport Base\ndef main():\n  Nat.pl<caret>")
      .exists(_.getLookupString == "plus"))

  def testBaseLawAndFillShareOneCompletionCandidate(): Unit =
    select(base("laws.bend", "law Equal.cong:\n  for n: Nat\n  {n == n : Nat}\n" +
      "def Equal.cong(n):\n  n\ndef Equal.compare():\n  0\n"))
    val found = items("import Base\ndef main():\n  Equal.co<caret>")
  ...
  def testDefaultsHomeExpansionAndUnverifiedCompatibility(): Unit =
    val defaults = BendToolchainPaths.resolve(BendToolchainChoices(), "/home/tester", None, 4)
    ...
    val overridePaths = BendToolchainPaths.resolve(
      BendToolchainChoices(executable = "~/bin/bend", baseSource = "~/lib/base.bend"),
      "/home/tester", Some("~/cache"), 5)
