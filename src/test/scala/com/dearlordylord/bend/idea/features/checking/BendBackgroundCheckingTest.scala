...
final class BendBackgroundCheckingTest extends BasePlatformTestCase:
  ...
  private def id(file: VirtualFile): FileId =
    new FileId(Option(file.getCanonicalPath).getOrElse(file.getPath),
      file.getCanonicalPath != null)

  private def awaitResult(root: FileId, predicate: BendCheckResult => Boolean): BendCheckResult =
    ...
    found.getOrElse(throw new AssertionError("No matching result for " + root.value +
      "; last=" + service.result(root).map(r => (r.fresh, r.details.take(300)))))

  private def slowCompiler(name: String, marker: Path, delaySeconds: Int): Path =
    val pinned = Path.of(".references/bend/bend2/main.ts").toAbsolutePath.normalize()
    val selectedBase = directory.resolve("base.bend")
    ...
    Files.writeString(executable,
      "#!/bin/sh\n" +
        ...
        "if [ \"$1\" = \"base\" ]; then cat '" + selectedBase + "'; exit $?; fi\n" +
        "printf x >> '" + marker + "'\n" +
        "sleep " + delaySeconds + "\n" +
        "exec npx --yes bun '" + pinned + "' \"$@\"\n")
    executable.toFile.setExecutable(true)
  ...
  def testTwoRootsKeepIndependentDiagnosticsOnOneDependency(): Unit =
    val dependency = myFixture.addFileToProject("shared.bend",
      "import Base\ndef value() -> U32:\n  unknown_shared\n")
    val first = myFixture.addFileToProject("first.bend",
    ...
    assertEquals(2, myFixture.doHighlighting().toArray.count(_.toString.contains("unknown_shared")))
    myFixture.openFileInEditor(first.getVirtualFile)
    ...
    assertEquals(1, service.resultsFor(dep).count(r => r.fresh &&
      r.diagnostics.exists(_.message.contains("unknown_shared"))))
    myFixture.openFileInEditor(dependency.getVirtualFile)
    assertEquals(1, myFixture.doHighlighting().toArray.count(_.toString.contains("unknown_shared")))

  def testRapidEditsAndDependencyOnlyEditPublishLatestBackgroundResult(): Unit =
    val dep = myFixture.addFileToProject("value.bend",
      "import Base\ndef value() -> U32:\n  1\n")
    myFixture.configureByText("root.bend",
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText(
        "import Base\ndef value() -> U32:\n  unknown_dependency\n"))
    val changed = awaitResult(root, r => r.fresh &&
  ...
  def testDisablingAndDisposalCancelPendingWork(): Unit =
    myFixture.configureByText("pending.bend", "import Base\ndef main() -> U32:\n  0\n")
    val root = id(myFixture.getFile.getVirtualFile)
    val settings = ApplicationManager.getApplication.getService(classOf[BendToolchainSettings])
    ...
    settings.update(settings.choices.copy(diagnosticsEnabled = false))
    Thread.sleep(750)
    assertTrue(getProject.getService(classOf[BendCheckService]).result(root).isEmpty)

  def testCanceledBackgroundProcessCannotPublishAfterNewEdit(): Unit =
    val settings = ApplicationManager.getApplication.getService(classOf[BendToolchainSettings])
    val marker = directory.resolve("started")
    val slow = slowCompiler("slow-bend", marker, 3)
    ...
    while !Files.exists(marker) && System.nanoTime() < started do Thread.sleep(25)
    assertTrue("The first process must start", Files.exists(marker))
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText(
        "import Base\ndef main() -> U32:\n  unknown_new\n"))
    val checked = awaitResult(root, r => r.fresh && r.details.contains("unknown_new"))
    assertFalse(checked.details.contains("unknown_old"))
    assertTrue("Edit must start a replacement process", Files.size(marker) >= 2L)

  def testManualActionPreemptsRunningBackgroundProcess(): Unit =
    val settings = ApplicationManager.getApplication.getService(classOf[BendToolchainSettings])
    val marker = directory.resolve("manual-started")
    val slow = slowCompiler("manual-slow-bend", marker, 3)
    ...
    myFixture.performEditorAction("Bend.CheckCurrentFile")
    val checked = awaitResult(root, r => r.fresh && r.details.contains("unknown_manual"))
  ...
  def testDependencyEditDuringFirstRootCheckSchedulesReplacement(): Unit =
    ...
    val slow = slowCompiler("dependency-slow-bend", marker, 3)
    val dep = myFixture.addFileToProject("early.bend",
      "import Base\ndef value() -> U32:\n  unknown_old_dependency\n")
    myFixture.openFileInEditor(dep.getVirtualFile)
    myFixture.configureByText("early_root.bend",
    ...
    val started = System.nanoTime() + 10_000_000_000L
    val service = getProject.getService(classOf[BendCheckService])
    while (!Files.exists(marker) || service.status(root) != BendCheckingStatus.Checking) &&
        System.nanoTime() < started do Thread.sleep(25)
    assertTrue("First graph check must reach the compiler", Files.exists(marker))
    assertEquals("Marker must belong to the root process",
      BendCheckingStatus.Checking, service.status(root))
    assertTrue("No root result has published yet",
      service.result(root).isEmpty)
    myFixture.openFileInEditor(dep.getVirtualFile)
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText(
        "import Base\ndef value() -> U32:\n  unknown_new_dependency\n"))
    val checked = awaitResult(root, r => r.fresh && r.details.contains("unknown_new_dependency"))
  ...
  def testCompilerReplacementDuringFirstCheckRetriesWithoutVfsEvent(): Unit =
    ...
    val slow = slowCompiler("replacement-bend", marker, 2)
    ...
    val checked = awaitResult(root, r => r.fresh &&
      r.details.contains("unknown_replacement"))
    assertEquals(BendCheckOutcome.Failed, checked.outcome)
    assertTrue("Rejected first result must trigger another compiler invocation",
  ...
  private def exerciseActiveDisposal(): Unit =
    ...
    val slow = slowCompiler("dispose-slow-bend", marker, 10)
    settings.update(settings.choices.copy(executable = slow.toString,
    ...
    assertTrue("Disposed session must drop all results", transient.result(root).isEmpty)
