...
final class BendImportCompletionTest extends BasePlatformTestCase:
  ...
  override def setUp(): Unit =
    ...
    settings.update(
      BendToolchainChoices(
        baseSource = temporary.resolve("base.bend").toString,
  ...
  def testDirectAliasAndDottedSuffix(): Unit =
    ...
    assertTrue(
      ...
      myFixture.getEditor.getDocument.getText.endsWith("L.Nat.add.extra")
    )
  ...
  def testCurrentFileDeclarationWinsOverImportedQualifiedName(): Unit =
    val lib =
      myFixture.addFileToProject("lib.bend", "def own(x: U32) -> U32:\n  x\n")
    myFixture.configureByText(
      "main.bend",
      "import ./lib.bend as L\ndef L.own():\n  0\ndef main():\n  L.ow<caret>"
    )
    ...
    catalog.resolve(
      ...
      case other =>
        fail(s"Expected imported target for alias collision, got $other")
  ...
  def testAliasFallsBackToLiteralDottedDeclarationWhenTargetAbsent(): Unit =
    ...
    resolution match
      case BendSourceResolution.Resolved(symbol) =>
        assertEquals(
          BendSourceSymbols.fileId(myFixture.getFile),
          symbol.handle.file
        )
  ...
  def testLastDirectImportOwnsDuplicateAlias(): Unit =
    ...
    val names = Option(myFixture.completeBasic())
      .getOrElse(Array.empty[LookupElement])
  ...
  def testUnsavedDependencyWinsAndInvalidatesParsedSymbols(): Unit =
    ...
    val document =
      FileDocumentManager.getInstance().getDocument(lib.getVirtualFile)
    WriteCommandAction.runWriteCommandAction(
      getProject,
      new Runnable:
        override def run(): Unit =
          document.setText("def newName(x: U32) -> U32:\n  x\n")
    )
    myFixture.configureByText(
      "main.bend",
      "import ./lib.bend as L\ndef main():\n  L.new<caret>"
