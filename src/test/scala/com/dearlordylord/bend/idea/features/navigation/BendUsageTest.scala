...
final class BendUsageTest extends BasePlatformTestCase:
  ...
  private def usages(element: PsiElement): List[PsiReference] =
    ReferencesSearch.search(element).findAll().asScala.toList
      .filter(r => r.getElement.getContainingFile.getLanguage ==
        com.dearlordylord.bend.idea.syntax.BendLanguage.instance)


  def testShadowedBinderAndAbsentUsage(): Unit =
    val outer = target("def f(x: U32) -> U32:\n  use(x)\n  let x = 1\n  use(<caret>x)\n")
  ...
  def testAliasesMembersAndSameNamesAcrossModules(): Unit =
    ...
    myFixture.addFileToProject("other.bend", "import ./a.bend as X\ndef other():\n  X.value()\n")
    val member = target("import ./a.bend as A\nimport ./b.bend as B\ndef main():\n  A.<caret>value()\n  B.value()\n")
    val memberUsages = usages(member)
    assertEquals(2, memberUsages.size)
    assertEquals(Set("A.value", "X.value"), memberUsages.map(_.getElement.getText).toSet)
    assertTrue(memberUsages.forall(_.getRangeInElement.getStartOffset == 2))
    val alias = target("import ./a.bend as A\ndef main():\n  <caret>A.value()\n  A.value()\n")
  ...
  def testUnsavedImportedDocumentTakesPrecedence(): Unit =
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText("def fresh():\n  0\n"))
    PsiDocumentManager.getInstance(getProject).commitAllDocuments()
    val imported = target("import ./lib.bend as L\ndef main():\n  L.<caret>fresh()\n")
  ...
  def testLargeSavedFileUsesShortUnsavedDocument(): Unit =
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText("def fresh():\n  0\n"))
    PsiDocumentManager.getInstance(getProject).commitAllDocuments()
    assertTrue("Saved VFS size must exceed the search limit", lib.getVirtualFile.getLength > 1024 * 1024)
    val imported = target("import ./large.bend as L\ndef main():\n  L.<caret>fresh()\n")
    assertEquals(1, usages(imported).size)

  def testStandardHighlightUsagesAction(): Unit =
    target("def value():\n  0\ndef main():\n  <caret>value()\n  value()\n")
    myFixture.performEditorAction("HighlightUsagesInFile")
    val highlights = myFixture.getEditor.getMarkupModel.getAllHighlighters.toList
    ...
    assertTrue("First resolved usage should be highlighted", highlights.exists(_.getStartOffset == first))
    assertTrue("Second resolved usage should be highlighted", highlights.exists(_.getStartOffset == second))

  def testDeclarationOwnerAndForwardReference(): Unit =
    val forward = target("def main():\n  <caret>later()\ndef later():\n  0\n")
    assertEquals(1, usages(forward).size)
    myFixture.configureByText("owner.bend", "def value():\n  0\ndef main():\n  value()\n")
    val owner = PsiTreeUtil.findChildOfType(myFixture.getFile, classOf[BendDefinition])
  ...
  def testLocalScopeRestrictsToSelectedSubtree(): Unit =
    val declaration = target("def value():\n  0\ndef first():\n  <caret>value()\ndef second():\n  value()\n")
    val file = myFixture.getFile
    val first = PsiTreeUtil.findChildrenOfType(file, classOf[BendDefinition]).asScala
      .find(_.getName == "first").get
    val scoped = ReferencesSearch.search(declaration,
      new LocalSearchScope(Array[PsiElement](first))).findAll().asScala.toList
    assertEquals(1, scoped.size)
    assertEquals(first, PsiTreeUtil.getParentOfType(scoped.head.getElement, classOf[BendDefinition]))

  def testUnusedLambdaBinderStillHasSourceIdentity(): Unit =
    myFixture.configureByText("lambda.bend", "def main():\n  use(<caret>x => 1)\n")
    val offset = myFixture.getEditor.getCaretModel.getOffset
    assertTrue(BendSourceSymbols.bindingDeclaredAt(myFixture.getFile, offset).nonEmpty)
