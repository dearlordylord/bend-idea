...
final class BendDocumentationTest extends BasePlatformTestCase:
  ...
  private def docs(source: String): String =
    ...
    val element = provider.getCustomDocumentationElement(myFixture.getEditor, file,
      file.findElementAt(offset), offset)
    if element == null then null else provider.generateDoc(element, file.findElementAt(offset))

  def testLocalSignatureCommentsAndLink(): Unit =
    val html = docs("# Explain & map\ndef map(~f: U32 -> U32, +xs: List<&2, U32>) -> List<&2, U32>:\n  xs\ndef main():\n  <caret>map(0, 0)\n")
    assertNotNull(html)
  ...
  def testImportedAndUnsavedSource(): Unit =
    val lib = myFixture.addFileToProject("lib.bend", "# Old\ndef value(x: U32) -> U32:\n  x\n")
    myFixture.openFileInEditor(lib.getVirtualFile)
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = myFixture.getEditor.getDocument.setText("# New\ndef value(+x: U32) -> U32:\n  x\n"))
    PsiDocumentManager.getInstance(getProject).commitAllDocuments()
    val html = docs("import ./lib.bend as L\ndef main():\n  L.<caret>value(1)\n")
    ...
    val target = new BendDocumentationProvider().getDocumentationElementForLink(
      PsiManager.getInstance(getProject), url,
  ...
  def testLawFillShowsSpecificationAndImplementationNames(): Unit =
    ...
    val provider = new BendDocumentationProvider
    val context = myFixture.getFile.findElementAt(myFixture.getEditor.getCaretModel.getOffset)
    val links = "href='([^']+)'>([^<]+)</a>".r.findAllMatchIn(html)
      .map(m => m.group(2) -> m.group(1)).toMap
    val manager = PsiManager.getInstance(getProject)
    val declaration = provider.getDocumentationElementForLink(manager, links("Declaration"), context)
    val implementation = provider.getDocumentationElementForLink(manager, links("Implementation"), context)
  ...
  def testImportedLawAndProofFillKeepRootContextAndPhysicalLinks(): Unit =
    ...
    val provider = new BendDocumentationProvider
    val manager = PsiManager.getInstance(getProject)
    assertEquals(laws.getVirtualFile,
      provider.getDocumentationElementForLink(manager, links("Declaration"), context)
        .getContainingFile.getVirtualFile)
    assertEquals(myFixture.getFile.getVirtualFile,
      provider.getDocumentationElementForLink(manager, links("Implementation"), context)
        .getContainingFile.getVirtualFile)
    val fillDoc = docs("import ./LAWS.bend as Laws\ndef Laws.<caret>claim(value):\n  value\n")
  ...
  def testStaleDocumentationLinkDoesNotNavigateSameOffsetRename(): Unit =
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit =
        ...
        document.replaceString(document.getText.indexOf("def foo") + 4,
          document.getText.indexOf("def foo") + 7, "bar"))
    PsiDocumentManager.getInstance(getProject).commitAllDocuments()
    val context = myFixture.getFile.findElementAt(myFixture.getEditor.getCaretModel.getOffset)
    assertNull(provider.getDocumentationElementForLink(PsiManager.getInstance(getProject),
      url, context))

  def testImportedSourceLawAndFillStayPaired(): Unit =
    val lib = myFixture.addFileToProject("paired.bend",
      "# Imported law\nlaw claim:\n  for x: Nat\n  {x == x : Nat}\n# Imported fill\ndef claim(value):\n  value\n")
    val html = docs("import ./paired.bend as P\ndef main():\n  P.<caret>claim(1)\n")
    ...
    val provider = new BendDocumentationProvider
    val manager = PsiManager.getInstance(getProject)
    assertEquals(lib.getVirtualFile,
      provider.getDocumentationElementForLink(manager, links("Declaration"), context)
        .getContainingFile.getVirtualFile)
    assertEquals(lib.getVirtualFile,
      provider.getDocumentationElementForLink(manager, links("Implementation"), context)
    ...
    val target = provider.getCustomDocumentationElement(myFixture.getEditor,
      myFixture.getFile, myFixture.getFile.findElementAt(fill), fill)
    assertTrue(provider.generateDoc(target, myFixture.getFile.findElementAt(fill))
      .contains("for x: Nat"))
  ...
  def testShadowedDirectAliasUsesLastImportForLawPair(): Unit =
    myFixture.addFileToProject("first.bend",
      "# First law\nlaw claim:\n  Type\n")
    val second = myFixture.addFileToProject("second.bend",
      "# Second law\nlaw claim:\n  for x: Nat\n  Type\n")
    val html = docs("import ./first.bend as L\nimport ./second.bend as L\ndef L.claim(value):\n  value\ndef main():\n  L.<caret>claim(1)\n")
    assertTrue(html.contains("Second law"))
    assertFalse(html.contains("First law"))
    assertTrue(html.contains("def L.claim(value)"))
    val url = "href='([^']+)'>([^<]+)</a>".r.findAllMatchIn(html)
      .find(_.group(2) == "Declaration").get.group(1)
    val target = new BendDocumentationProvider().getDocumentationElementForLink(
      PsiManager.getInstance(getProject), url,
      myFixture.getFile.findElementAt(myFixture.getEditor.getCaretModel.getOffset))
  ...
  def testNamespaceConflictCannotPairRejectedAliasWithLaw(): Unit =
    ...
    Files.writeString(target, "# Accepted law\nlaw claim:\n  for x: Nat\n  Type\n")
    Files.createSymbolicLink(alias, target.getFileName)
    val imports = s"import $target as Accepted\nimport $alias as Rejected\n"
    val rejected = docs(imports + "def Rejected.<caret>claim(value):\n  value\n")
    val graph = getProject.getService(classOf[com.dearlordylord.bend.idea.symbols.api.BendImportedSymbolCatalog])
    val (basePath, packageCache) = getProject.getService(
      classOf[com.dearlordylord.bend.idea.workspace.api.BendLoadingConfiguration]).paths
    assertTrue(graph.loaded(myFixture.getFile, basePath, packageCache).problems.exists(
      _.isInstanceOf[com.dearlordylord.bend.idea.workspace.model.BendGraphProblem.NamespaceConflict]))
    ...
    val accepted = docs(imports + "def Accepted.<caret>claim(value):\n  value\n")
