...
final class BendStructureTest extends BasePlatformTestCase:
  def testOutlineLabelsAndNavigation(): Unit =
    ...
    try
      ...
      assertTrue(
        declarations.head.getPresentation.getPresentableText
          .startsWith("def first")
      )
      assertTrue(
        declarations(1).getPresentation.getPresentableText
          .startsWith("law claim")
      )
      assertTrue(
        declarations(1).getPresentation.getPresentableText.length <= 120
      )
      assertTrue(
        declarations(2).getPresentation.getPresentableText
          .startsWith("type Pair")
      )
      assertTrue(
        declarations(3).getPresentation.getPresentableText
          .startsWith("def last")
      )
      val constructors = declarations(2).getChildren.toList
      assertEquals(1, constructors.size)
      assertTrue(
        constructors.head.getPresentation.getPresentableText.startsWith("Pair{")
      )
      val constructor = constructors.head
        .asInstanceOf[com.intellij.ide.structureView.StructureViewTreeElement]
      assertEquals(
  ...
  def testMalformedAndCommentedDeclarationsDoNotHideNeighbors(): Unit =
    ...
    val model = builder.createStructureViewModel(myFixture.getEditor)
    try
      val names = model.getRoot.getChildren.toList
