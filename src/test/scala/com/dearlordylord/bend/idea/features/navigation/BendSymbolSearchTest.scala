...
import com.dearlordylord.bend.idea.symbols.index.{BendChooseByNameContributor, BendDottedComponentIndex}
...
final class BendSymbolSearchTest extends BasePlatformTestCase:
  ...
  override def setUp(): Unit =
    ...
    val base = Files.writeString(temporary.resolve("base.bend"), "def baseOnly():\n  0\n")
    val cache = Files.createDirectories(temporary.resolve("cache/0xabc"))
    val cachedFile = Files.writeString(cache.resolve("module.bend"), "def cachedOnly():\n  0\n")
  ...
  private def items(name: String): List[NavigationItem] =
    ...
    contributor.processElementsWithName(name, new Processor[NavigationItem]:
  ...
  def testGlobalSearchFindsUnimportedBendDeclarationsByName(): Unit =
    ...
    val dotted = myFixture.addFileToProject("alias.bend", "def M.U32.add():\n  0\n")
    assertTrue(StubIndex.getElements(BendDottedComponentIndex.Key, "U32.add", getProject,
      GlobalSearchScope.allScope(getProject), classOf[com.dearlordylord.bend.idea.syntax.psi.BendDeclaration])
      .asScala.exists(_.getContainingFile.getVirtualFile == dotted.getVirtualFile))

  def testDuplicateNamesRemainDistinctAndShowTheirSourceFiles(): Unit =
    val first = myFixture.addFileToProject("first.bend", "def same():\n  0\n")
    val second = myFixture.addFileToProject("second.bend", "law same:\n  Type\n")

    val found = items("same")
    assertEquals(2, found.size)
    assertEquals(Set(first.getVirtualFile, second.getVirtualFile),
      found.map(_.asInstanceOf[com.intellij.psi.PsiElement].getContainingFile.getVirtualFile).toSet)
    assertEquals(Set("def same", "law same"), found.map(_.getPresentation.getPresentableText).toSet)
  ...
  def testOpenUnsavedBufferReplacesItsIndexedDeclarations(): Unit =
    ...
    WriteCommandAction.runWriteCommandAction(getProject, new Runnable:
      override def run(): Unit = editor.getDocument.setText("def after():\n  1\n"))
    ...
    assertEquals(file.getVirtualFile, items("after").head.asInstanceOf[com.intellij.psi.PsiElement]
  ...
  def testChangingConfiguredLibraryRootsChangesSearchResults(): Unit =
    ...
    val nextBase = Files.writeString(temporary.resolve("replacement/base.bend"),
      "def replacementBase():\n  0\n")
    val nextCached = Files.writeString(replacement.resolve("module.bend"),
    ...
    ApplicationManager.getApplication.getService(classOf[BendToolchainSettings])
      .update(BendToolchainChoices(baseSource = nextBase.toString,
        packageCache = temporary.resolve("replacement/cache").toString))
    assertTrue(BendConfiguredSymbolRoots.roots(getProject).exists(_.getPath == nextBase.toString))
    assertTrue(com.intellij.openapi.roots.ProjectFileIndex.getInstance(getProject)
      .isInLibrarySource(LocalFileSystem.getInstance().findFileByPath(nextBase.toString)))
    assertTrue(com.intellij.openapi.roots.ProjectFileIndex.getInstance(getProject)
      .isInLibrarySource(LocalFileSystem.getInstance().findFileByPath(nextCached.toString)))
    IndexingTestUtil.waitUntilIndexesAreReady(getProject)
  ...
  def testOpenBufferRemainsSearchableWhileIndexesAreUnavailable(): Unit =
    val file = myFixture.addFileToProject("open.bend", "def localWhileDumb():\n  0\n")
    myFixture.openFileInEditor(file.getVirtualFile)
    ...
    DumbModeTestUtils.computeInDumbModeSynchronously(getProject,
      new ThrowableComputable[Unit, Throwable]:
        override def compute(): Unit =
          ...
          assertEquals(file.getVirtualFile, items("localWhileDumb").head
            .asInstanceOf[com.intellij.psi.PsiElement].getContainingFile.getVirtualFile))
