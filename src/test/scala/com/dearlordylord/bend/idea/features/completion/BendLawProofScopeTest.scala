...
final class BendLawProofScopeTest extends BasePlatformTestCase:
  private def bindings(source: String): Set[String] =
    ...
    BendSourceSymbols.visibleBindings(myFixture.getFile,
      myFixture.getEditor.getCaretModel.getOffset).map(_.name).toSet
  ...
  def testLawFillKeepsBothSourceSitesAndOneCompletion(): Unit =
    ...
    assertEquals(1, BendSourceSymbols.visibleCandidates(myFixture.getFile, offset).count(_.name == "claim"))
    assertTrue(items.length == 1 || myFixture.getEditor.getDocument.getText.substring(0, offset).endsWith("claim"))
    val sites = BendSourceSymbols.declarations(myFixture.getFile).filter(_.name == "claim")
    assertEquals(List(BendSymbolCategory.Law, BendSymbolCategory.Definition), sites.map(_.category))
    ...
    if items.nonEmpty then
      assertTrue(items.head.getObject.isInstanceOf[com.dearlordylord.bend.idea.syntax.psi.BendLaw])
    assertEquals(BendSourceResolution.Resolved(sites.last), BendSourceSymbols.resolveCurrentFile(
      myFixture.getFile, offset, "claim", Some(BendSymbolCategory.Definition)))
  ...
  def testLawClauseDomainsWhereAndOrdering(): Unit =
    ...
    assertTrue(bindings("law proof:\n  for n: Nat\n  for m: <caret>Nat\n  {n == m : Nat}").contains("n"))
    assertEquals(Set("n", "m"), bindings("law proof:\n  for n: Nat\n  exs m: Nat\n  {n == <caret>m : Nat}"))
    assertTrue(bindings("law proof:\n  for n: Nat where {n == n : Nat}\n  <caret>Type").contains("n"))
    assertFalse(bindings("law proof:\n  for n: <caret>Nat where {n == n : Nat}\n  Type").contains("n"))
    assertTrue(bindings("law proof:\n  for n: Nat where {<caret>n == n : Nat}\n  Type").contains("n"))
    assertEquals(Set("n", "m"), bindings(
      "law proof: for n: Nat for m: Nat\n  {n == <caret>m : Nat}"))
  ...
  def testDependentArrowAndMotiveBoundaries(): Unit =
    ...
    assertTrue(bindings("def f(g: @x: Nat -> {<caret>x == x : Nat}) -> Nat:\n  0").contains("x"))
    assertFalse(bindings("def f(g: @x: Nat -> Nat) -> {<caret>x == x : Nat}:\n  0").contains("x"))
    assertFalse(bindings("def f(g: @x: Nat -> Nat) -> Nat:\n  <caret>0").contains("x"))
    assertEquals(Set("n", "e", "_"), bindings("law proof:\n  for n: Nat\n  {n == n : Nat}\ndef proof(n):\n  %e@n : {_<caret> == n : Nat}; {==}"))
    assertFalse(bindings("law proof:\n  for n: Nat\n  {n == n : Nat}\ndef proof(n):\n  %e@n : {_ == n : Nat}; <caret>{==}").contains("e"))

  def testNamedAndTodoHolesAreNotDeclarations(): Unit =
    candidates("law proof:\n  Type\ndef proof():\n  ?named\n  ?TODO\ndef main():\n  pro<caret>")
  ...
  def testBareFillParameterUsesLawTypeAndKeepsFillLocator(): Unit =
    myFixture.configureByText("proof.bend",
      "law claim:\n  for -n: Nat\n  {n == n : Nat}\ndef claim(n):\n  n<caret>\n")
    val bindings = BendSourceSymbols.visibleBindings(myFixture.getFile,
      myFixture.getEditor.getCaretModel.getOffset)
    val n = bindings.find(_.name == "n").get
    assertEquals(Some("Nat"), n.sourceSpecification)
    assertEquals(Some(BendSourceSymbols.logicalLaws(myFixture.getFile).head.law.handle), n.specification)
    assertEquals(myFixture.getFile.getText.indexOf("def claim(n)") + "def claim(".length,
      n.handle.nameOffset)

  def testWhereClauseRemainsInBareFillSpecification(): Unit =
    val items = candidates("law my_thesis:\n  for x: Nat where IsEven(x)\n  Type\ndef my_thesis(x):\n  <caret>?TODO")
    val x = BendSourceSymbols.visibleBindings(myFixture.getFile,
      myFixture.getEditor.getCaretModel.getOffset).find(_.name == "x").get
    assertEquals(Some("Nat where IsEven(x)"), x.sourceSpecification)
    assertEquals(Some(BendSourceSymbols.logicalLaws(myFixture.getFile).head.law.handle), x.specification)
  ...
  def testTypedDefinitionAfterLawIsNotItsFill(): Unit =
    candidates("law claim:\n  Type\ndef claim(f: Nat -> Nat) -> IO(Unit):\n  ?TODO\ndef main():\n  cla<caret>")
    assertTrue(BendSourceSymbols.logicalLaws(myFixture.getFile).head.isOpen)
    assertEquals(2, BendSourceSymbols.visibleCandidates(myFixture.getFile,
      myFixture.getEditor.getCaretModel.getOffset).count(_.name == "claim"))
    candidates("law claim:\n  Type\ndef claim(f: Nat -> Nat):\n  ?TODO\ndef main():\n  cla<caret>")
    assertTrue(BendSourceSymbols.logicalLaws(myFixture.getFile).head.isOpen)

  def testProofFormsRemainDistinctThroughIncompleteEdit(): Unit =
    candidates("law claim:\n  {0n == 0n : Nat}\ndef claim():\n  %e@{0n == 0n : Nat} : {_ == 0n : Nat};\n  (1 + 2 : U32)\n  ?named\n  ?TODO\n  ?\n  {==}\n  %incomplete : {")
    val forms = BendSourceSymbols.declarations(myFixture.getFile).last.declaration.proofForms
    assertEquals(Set("named", "TODO", ""), forms.collect { case h: BendProofForm.Hole => h.name }.toSet)
    ...
    assertTrue(forms.exists { case r: BendProofForm.Rewrite => r.motive.nonEmpty; case _ => false })
