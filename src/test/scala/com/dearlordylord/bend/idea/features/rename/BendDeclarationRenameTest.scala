...
final class BendDeclarationRenameTest extends BasePlatformTestCase:
  ...
  def testImportedMemberRenameKeepsAlias(): Unit =
    val library = myFixture.addFileToProject("lib.bend", "def value() -> U32:\n  1\n")
    myFixture.configureByText("main.bend",
  ...
  def testLawRenameKeepsTwoIndependentProofRoots(): Unit =
    val first = myFixture.addFileToProject("proofA.bend",
      "import ./LAWS.bend as Laws\ndef Laws.claim():\n  1\ndef run():\n  Laws.claim()\n")
    val second = myFixture.addFileToProject("proofB.bend",
      "import ./LAWS.bend as Laws\ndef Laws.claim():\n  2\ndef run():\n  Laws.claim()\n")
    myFixture.configureByText("LAWS.bend", "law <caret>claim:\n  {1 == 1 : U32}\n")
    myFixture.renameElementAtCaret("assertion")
    assertEquals("law assertion:\n  {1 == 1 : U32}\n", myFixture.getEditor.getDocument.getText)
    ...
    val editor = FileEditorManager.getInstance(getProject).getSelectedEditor(myFixture.getFile.getVirtualFile)
  ...
  def testDeclarationCollisionIsRejectedBeforeEdit(): Unit =
    ...
    myFixture.configureByText("main.bend", source.replace("first", "<caret>first"))
    var rejected = false
    try myFixture.renameElementAtCaret("second")
    catch case _: BaseRefactoringProcessor.ConflictsInTestsException => rejected = true
    assertTrue(rejected)
    assertEquals(source, myFixture.getEditor.getDocument.getText)

  def testDeclarationRenameRejectsCaptureByLocalBinder(): Unit =
    val source = "def source() -> U32:\n  1\ndef use(item: U32) -> U32:\n  source()\n"
    myFixture.configureByText("capture.bend", source.replace("def source", "def <caret>source"))
    var rejected = false
  ...
  def testDeclarationRenameRejectsCaptureOfUnresolvedCall(): Unit =
    val source = "def source() -> U32:\n  1\ndef use() -> U32:\n  item()\ndef main() -> U32:\n  source()\n"
    myFixture.configureByText("unresolved.bend", source.replace("def source", "def <caret>source"))
    var rejected = false
    try myFixture.renameElementAtCaret("item")
    catch case _: BaseRefactoringProcessor.ConflictsInTestsException => rejected = true
    assertTrue(rejected)
  ...
  def testDeclarationRenameAllowsUnrelatedLocalBinder(): Unit =
    myFixture.configureByText("safe.bend",
  ...
  def testRenameFromQualifiedFillKeepsItsAlias(): Unit =
    val law = myFixture.addFileToProject("LAWS.bend", "law claim:\n  {1 == 1 : U32}\n")
    val other = myFixture.addFileToProject("other.bend",
    ...
    assertEquals("import ./LAWS.bend as Laws\ndef Laws.assertion():\n  1\n",
      myFixture.getEditor.getDocument.getText)
    assertEquals("law assertion:\n  {1 == 1 : U32}\n", law.getText)
    assertEquals("import ./LAWS.bend as Proof\ndef Proof.assertion():\n  2\n", other.getText)

  def testLawRenameRejectsCollisionInAnotherProofRoot(): Unit =
    val source = "law claim:\n  {1 == 1 : U32}\n"
    val proof = myFixture.addFileToProject("proof.bend",
      "import ./LAWS.bend as Laws\ndef Laws.claim():\n  1\ndef Laws.assertion() -> U32:\n  2\n")
    myFixture.configureByText("LAWS.bend", source.replace("claim", "<caret>claim"))
    var rejected = false
  ...
  def testReadonlyLawBlocksFillRename(): Unit =
    ...
    try
      ...
      assertEquals("import ./LAWS.bend as Laws\ndef Laws.claim():\n  1\n",
        myFixture.getEditor.getDocument.getText)
    finally ApplicationManager.getApplication.runWriteAction(new Runnable:
  ...
  def testReadonlyCachedLawBlocksFillRename(): Unit =
    ...
    val law = Files.createDirectories(directory.resolve("cache/0xabc")).resolve("LAWS.bend")
    Files.writeString(law, "law claim:\n  {1 == 1 : U32}\n")
    val virtual = LocalFileSystem.getInstance().refreshAndFindFileByPath(law.toString)
    assertNotNull(virtual)
    val writable = virtual.isWritable
    settings.update(BendToolchainChoices(packageCache = directory.resolve("cache").toString))
    ApplicationManager.getApplication.runWriteAction(new Runnable:
      override def run(): Unit = virtual.setWritable(false))
    try
      myFixture.configureByText("proof.bend",
        "import 0xabc/LAWS.bend as Laws\ndef Laws.<caret>claim():\n  1\n")
      val (basePath, cachePath) = getProject.getService(
      ...
      val scan = BendSourceFiles.scan(myFixture.getFile,
        GlobalSearchScope.projectScope(getProject), includeConfiguredLibraries = true)
      assertTrue(scan.files.map(_.getName).mkString(","), scan.files.exists(_.getName == "LAWS.bend"))
      val selected = BendSourceSymbols.declarations(myFixture.getFile).find(_.name == "Laws.claim").get
      val related = BendSourceSymbols.relatedLawSites(scan.files, selected, basePath, cachePath)
      assertTrue(related.map(_.name).mkString(","),
        related.exists(symbol => symbol.category == BendSymbolCategory.Law &&
          !symbol.declaration.isWritable))
      var rejected = false
      try myFixture.renameElementAtCaret("assertion")
      catch case _: RuntimeException => rejected = true
      assertTrue(rejected)
      assertEquals("import 0xabc/LAWS.bend as Laws\ndef Laws.claim():\n  1\n",
        myFixture.getEditor.getDocument.getText)
  ...
  def testPinnedCompilerAcceptsBeforeAndAfterDeclarationRename(): Unit =
    ...
    try
      ...
      myFixture.configureByText("main.bend", source.replace("def value", "def <caret>value"))
      myFixture.renameElementAtCaret("item")
