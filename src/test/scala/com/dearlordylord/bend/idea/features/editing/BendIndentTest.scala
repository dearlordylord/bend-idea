...
final class BendIndentTest extends BasePlatformTestCase:
  ...
  def testBackspaceStepsThroughLeadingIndentOnly(): Unit =
    myFixture.configureByText("backspace.bend", "def f():\n  match x:\n    case Zero{}:\n      <caret>value")
    myFixture.performEditorAction(IdeActions.ACTION_EDITOR_BACKSPACE)
    assertEquals("def f():\n  match x:\n    case Zero{}:\n    value", myFixture.getEditor.getDocument.getText)
    myFixture.performEditorAction(IdeActions.ACTION_EDITOR_BACKSPACE)
    assertEquals("def f():\n  match x:\n    case Zero{}:\n  value", myFixture.getEditor.getDocument.getText)
  ...
  def testBackspaceInExpressionDoesNotChangeLeadingIndentOrOperators(): Unit =
    myFixture.configureByText("expression.bend", "def f():\n  value<caret> + other")
    myFixture.performEditorAction(IdeActions.ACTION_EDITOR_BACKSPACE)
    assertEquals("def f():\n  valu + other", myFixture.getEditor.getDocument.getText)

  def testEnterInsideInlineCallKeepsFollowingExpression(): Unit =
    assertEquals("def f(): call(\n  value)", enter("def f(): call(<caret>value)"))
