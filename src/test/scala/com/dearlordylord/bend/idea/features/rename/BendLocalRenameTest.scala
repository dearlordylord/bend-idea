...
final class BendLocalRenameTest extends BasePlatformTestCase:
  ...
  def testCollisionIsReportedBeforeEdit(): Unit =
    ...
    new BendLocalRenameProcessor().findExistingNameConflicts(
      binder,
      "y",
      conflicts
    )
    ...
    assertThrows(
      classOf[BaseRefactoringProcessor.ConflictsInTestsException],
  ...
  def testTelescopeRenameUpdatesLaterParameterType(): Unit =
    ...
    assertEquals(
      "def f(size: Nat, value: Vec<size>) -> Nat:\n  size\n",
      myFixture.getEditor.getDocument.getText
    )
  ...
  def testRewriteBinderRenameStaysInsideMotive(): Unit =
    ...
    assertTrue(
      com.dearlordylord.bend.idea.symbols.api.BendSourceSymbols
        .bindingDeclaredAt(file, names.head.getTextOffset)
        .nonEmpty
    )
