...
final class BendEditorTest extends BasePlatformTestCase:
  private def scheme(name: String, background: Color, foreground: Color, keyword: Color): EditorColorsScheme =
    val copy = EditorColorsManager.getInstance().getGlobalScheme.clone().asInstanceOf[EditorColorsScheme]
    copy.setName(name)
  ...
  private def assertEffectiveEditorColors(background: Color, foreground: Color, keyword: Color): Unit =
    ...
    assertEquals(foreground, editor.getColorsScheme.getAttributes(HighlighterColors.TEXT).getForegroundColor)
    val plainOffset = editor.getDocument.getText.indexOf("plain")
    assertEquals(foreground, editor.getHighlighter.createIterator(plainOffset).getTextAttributes.getForegroundColor)
    val keywordOffset = editor.getDocument.getText.indexOf("do")
    assertEquals(keyword, editor.getHighlighter.createIterator(keywordOffset).getTextAttributes.getForegroundColor)

  def testEffectiveEditorColorsFollowDarkAndLightSchemes(): Unit =
    val manager = EditorColorsManager.getInstance()
    val original = manager.getGlobalScheme
    val dark = scheme("Bend fixture dark", new Color(30, 33, 38), new Color(220, 224, 230), new Color(220, 150, 90))
    val light = scheme("Bend fixture light", new Color(250, 248, 244), new Color(35, 39, 45), new Color(90, 60, 160))
    try
      for (selected, background, foreground, keyword) <- Seq(
        (dark, new Color(30, 33, 38), new Color(220, 224, 230), new Color(220, 150, 90)),
        (light, new Color(250, 248, 244), new Color(35, 39, 45), new Color(90, 60, 160))) do
        ...
        myFixture.configureByText("scheme.bend", "def main(): do IO<Unit>: plain")
  ...
  def testOpenBendEditorUpdatesAfterLiveSchemeSwitch(): Unit =
    val manager = EditorColorsManager.getInstance()
    val original = manager.getGlobalScheme
    val dark = scheme("Bend live dark", new Color(24, 28, 34), new Color(230, 230, 220), new Color(190, 140, 210))
    val light = scheme("Bend live light", new Color(255, 253, 249), new Color(30, 35, 40), new Color(70, 80, 180))
    try
      ...
      assertEffectiveEditorColors(new Color(24, 28, 34), new Color(230, 230, 220), new Color(190, 140, 210))
      manager.setGlobalScheme(light)
      PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
      assertSame(editor, myFixture.getEditor)
      assertSame(document, editor.getDocument)
      assertEffectiveEditorColors(new Color(255, 253, 249), new Color(30, 35, 40), new Color(70, 80, 180))
    finally
      manager.setGlobalScheme(original)
      PlatformTestUtil.dispatchAllEventsInIdeEventQueue()
