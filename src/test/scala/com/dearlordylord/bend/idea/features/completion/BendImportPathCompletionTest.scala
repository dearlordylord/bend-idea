...
final class BendImportPathCompletionTest extends BasePlatformTestCase:
  ...
  override def setUp(): Unit =
    ...
    settings.update(BendToolchainChoices(packageCache = temporary.resolve("cache").toString))
  ...
  def testFilenameReplacesWholePathIncludingAfterCaret(): Unit =
    ...
    assertEquals("import ./sub/part.bend as P", myFixture.getEditor.getDocument.getText)
  ...
  def testDirectoryAcceptanceKeepsPathOpenForNextSegment(): Unit =
    ...
    assertEquals("import ./sub/deep/ as C", myFixture.getEditor.getDocument.getText)
    assertEquals("import ./sub/deep/".length, myFixture.getEditor.getCaretModel.getOffset)
    val next = items("import ./sub/deep/<caret> as C")
    assertTrue(next.exists(_.getLookupString == "./sub/deep/child.bend") ||
  ...
  def testCachedHashAndAbsolutePath(): Unit =
    ...
    assertTrue(cachedFile.exists(_.getLookupString == "0xabc/pkg.bend") ||
    ...
    assertTrue(absoluteItems.exists(_.getLookupString == absolute.toString + "/mod.bend") ||
  ...
  def testNormalizedHashParentReturnsToLocalDirectory(): Unit =
    ...
    assertTrue(found.exists(_.getLookupString == "0xabc/../foo.bend") ||
  ...
  def testOpenUnsavedModuleSurvivesSaturatedDirectory(): Unit =
    ...
    (0 until 260).foreach(i => Files.writeString(crowded.resolve(f"disk$i%03d.bend"), "def disk():\n  0\n"))
    val unsaved = crowded.resolve("unpersisted.bend")
    Files.writeString(unsaved, "def fresh():\n  0\n")
    val virtual = LocalFileSystem.getInstance().refreshAndFindFileByNioFile(unsaved)
    ...
    assertTrue(found.exists(_.getLookupString == crowded.toString + "/unpersisted.bend") ||
