...
final class BendCompletionTest extends BasePlatformTestCase:
  ...
  def testTopLevelAndBodyKeywords(): Unit =
    ...
    assertTrue(
      kind.contains("Data") || myFixture.getEditor.getDocument.getText
        .endsWith("Data")
    )
    ...
    assertTrue(
      cases.contains("case") || myFixture.getEditor.getDocument.getText
        .endsWith("case")
    )
  ...
  def testOrdinaryTypingAndIncompleteSourceStillComplete(): Unit =
    ...
    assertTrue(
      incomplete.contains("return") || myFixture.getEditor.getDocument.getText
        .endsWith("return")
    )
  ...
  def testAllSixSnippetsExpandThroughLookup(): Unit =
    ...
    for (trigger, expected, firstField) <- cases do
      val prefix =
        if trigger == "match" || trigger == "do" then "def main():\n  " else ""
      myFixture.configureByText(
        "snippet.bend",
        prefix + trigger.take(2) + "<caret>"
      )
      val items =
        Option(myFixture.completeBasic()).getOrElse(Array.empty[LookupElement])
      ...
      val indented =
        if prefix.nonEmpty then expected.replace("\n", "\n  ") else expected
      assertEquals(prefix + indented, editor.getDocument.getText)
      assertEquals(
        s"$trigger first editable field",
        firstField,
      ...
      while editor.getSelectionModel.hasSelection && steps < 12 do
        ...
        steps += 1
      assertFalse(
        s"$trigger template did not finish",
        editor.getSelectionModel.hasSelection
      )
      assertEquals(
        prefix.length + indented.length,
  ...
  def testEditingLawPlaceholderUpdatesLinkedOccurrences(): Unit =
    ...
    assertTrue(
      myFixture.getEditor.getDocument.getText.startsWith("law reflexive:")
    )
    myFixture.`type`("\t")
    assertEquals("x", myFixture.getEditor.getSelectionModel.getSelectedText)
    myFixture.`type`("value")
    assertTrue(
      myFixture.getEditor.getDocument.getText.contains("{value == value : Nat}")
    )
