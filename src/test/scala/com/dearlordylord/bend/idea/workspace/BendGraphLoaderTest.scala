...
final class BendGraphLoaderTest:
  private def record(path: String, text: String, canonical: String = ""): BendSourceRecord =
    BendSourceRecord(new FileId(if canonical.isEmpty then path else canonical, true),
      path, text, 1L, BendImportLines.parse(text))

  @Test def diamondDeduplicatesByCanonicalIdentityAndReportsNamespaceConflict(): Unit =
    ...
    assertTrue(graph.problems.exists(_.isInstanceOf[BendGraphProblem.NamespaceConflict]))
  ...
  @Test def cycleAndMissingRemainProblemsWithUsefulFiles(): Unit =
    ...
    val graph = BendGraphLoader.load(root, BendGraphLoader.Config("/base.bend", "/cache"),
      mapCatalog(root, a))
  ...
  @Test def duplicateNormalizedSpellingsWithSameNamespaceLoadOnce(): Unit =
    ...
    assertEquals(2, graph.files.size)
    assertEquals(List("./shared.bend", "././shared.bend"),
      graph.edges.map(_.importLine.spelling))
      ...
      val catalog = new BendSourceCatalog:
        override def source(path: String): Option[BendSourceRecord] =
          ...
          Option.when(Files.exists(requested)) {
            record(path, Files.readString(requested), requested.toRealPath().toString)
          }
  ...
  private def mapCatalog(records: BendSourceRecord*): BendSourceCatalog =
    ...
    new BendSourceCatalog:
      override def source(path: String): Option[BendSourceRecord] = byPath.get(path)
