...
/** Compiled dependency rules for A2/A4; semantic contracts still need
  * editor/process tests.
  */
private[architecture] object ArchitectureRules:
  ...
  private def allowed(
    ...
    then publicArea(target, "symbols", Set("api", "references"))
      ...
      from match
        case "symbols" =>
          to == "syntax" || publicArea(target, "workspace", Set("api", "model"))
        case "analysis" =>
          publicArea(target, "workspace", Set("api", "model")) ||
          publicArea(target, "toolchain", Set("api", "model"))
        case "features" =>
          to == "syntax" || publicArea(target, "symbols", Set("api")) ||
          publicArea(target, "workspace", Set("api", "model")) ||
          publicArea(target, "analysis", Set("api", "model")) ||
          publicArea(target, "toolchain", Set("api", "model"))
        case "adapters" =>
          to == "syntax" ||
          Set("workspace", "analysis", "toolchain").exists(owner =>
  ...
  def violations(classes: JavaClasses, root: String): Seq[String] =
    ...
    require(
      imported.nonEmpty,
      "Architecture rules must inspect compiled production classes"
    )
    imported.flatMap { cls =>
      ...
      val placement = Option
        ...
        .toSeq
      val dependencies =
        cls.getDirectDependenciesFromSelf.asScala.toSeq.flatMap { dependency =>
