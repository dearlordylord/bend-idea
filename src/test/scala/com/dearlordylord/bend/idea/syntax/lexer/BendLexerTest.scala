...
final class BendLexerTest:
  private case class Token(kind: IElementType, text: String, start: Int, end: Int, state: Int)

  private def tokens(source: String, offset: Int = 0, state: Int = 0): List[Token] =
    ...
    while lexer.getTokenType != null do
      ...
      result += Token(lexer.getTokenType, source.substring(previous, lexer.getTokenEnd), previous, lexer.getTokenEnd, lexer.getState)
  ...
  @Test def coversCrLfAstralAndInvalidInput(): Unit =
    ...
    assertTrue(found.exists(t => t.kind == BendTokens.StringContent && t.text == "😀" && t.end - t.start == 2))
    assertTrue(found.exists(t => t.kind == BendTokens.Escape && t.text == "\\u{1F600}"))
    assertTrue(found.exists(t => t.kind == BendTokens.InvalidEscape && t.text == "\\q"))
    assertTrue(found.exists(t => t.kind == BendTokens.Comment && t.text == "# end"))
    assertTrue(found.exists(t => t.kind == TokenType.BAD_CHARACTER && t.text == "§"))

  @Test def differentiatesLanguageVocabulary(): Unit =
    val found = tokens("import ./A.bend as A\nlaw L: for x: Nat\ndef f(&2 x: U32): {==} ?TODO\ntype T is Data:\ndo IO<Unit>: f!(~Nat)")
    def contains(kind: IElementType, value: String): Boolean = found.exists(t => t.kind == kind && t.text == value)
  ...
  @Test def unfinishedImportDoesNotColorNextLineAsPath(): Unit =
    val source = "import\r\ndef next(): ?TODO"
    val found = tokens(source)
    assertTrue(found.exists(t => t.kind == BendTokens.DeclarationKeyword && t.text == "def"))
    assertTrue(found.exists(t => t.kind == BendTokens.FunctionName && t.text == "next"))
    for boundary <- found do
      assertEquals(found.dropWhile(_.start < boundary.start), tokens(source, boundary.start, boundary.state))
  ...
  @Test def unfinishedDeclarationRecoversAtNextReservedWord(): Unit =
    ...
    assertTrue(found.exists(t => t.kind == BendTokens.FunctionName && t.text == "L"))
    assertTrue(found.exists(t => t.kind == BendTokens.FunctionName && t.text == "f"))
    assertTrue(found.exists(t => t.kind == BendTokens.FunctionName && t.text == "multiline"))
    for boundary <- found do
      assertEquals(found.dropWhile(_.start < boundary.start), tokens(source, boundary.start, boundary.state))
  ...
  @Test def trailingDotIsInvalidOverWholeLexemeAndAsIsImportOnly(): Unit =
    ...
    val found = tokens(source)
    assertTrue(found.exists(t => t.kind == TokenType.BAD_CHARACTER && t.text == "broken."))
    assertEquals(2, found.count(t => t.kind == BendTokens.Identifier && t.text == "as"))
    assertTrue(found.exists(t => t.kind == BendTokens.Keyword && t.text == "as"))
    assertTrue(found.exists(t => t.kind == BendTokens.NamespaceName && t.text == "B"))

  @Test def restartsFromEveryTokenBoundaryAndAfterLiteralEdit(): Unit =
    val original = "def greet() : \"line one\r\nline two 😀 \\n done\" # note\r\ndef next(): ?TODO"
    ...
    assertFalse(editedTokens.exists(t => t.kind == BendTokens.Comment && t.text.contains("def type")))
    assertTrue(editedTokens.exists(t => t.kind == BendTokens.StringContent && t.text.contains("line # def type")))
