...
import com.intellij.formatting.{Block, ChildAttributes, FormattingContext, FormattingModel, FormattingModelBuilder, FormattingModelProvider, Indent, Spacing}
import com.intellij.lang.ASTNode
import com.intellij.psi.TokenType
import com.intellij.psi.formatter.common.AbstractBlock
import scala.jdk.CollectionConverters.*

/** The surface parser deliberately leaves expressions opaque. Only punctuation gaps whose
  * existing line stays intact are eligible for formatting; every other gap is read-only.
  */
final class BendFormattingModelBuilder extends FormattingModelBuilder:
  override def createModel(context: FormattingContext): FormattingModel =
    ...
    val root = new BendFormatBlock(context.getNode, source,
      BendFormatSafety.complete(source) && BendFormatSafety.headersComplete(context.getNode))
    FormattingModelProvider.createFormattingModelForPsiFile(
      context.getContainingFile, root, context.getCodeStyleSettings)

private final class BendFormatBlock(node: ASTNode, source: String, complete: Boolean,
    indent: Indent = Indent.getNoneIndent())
    extends AbstractBlock(node, null, null):
  private val simpleBody = complete && BendFormatSafety.singleBodyLine(myNode, source)
  ...
  override protected def buildChildren(): java.util.List[Block] =
    ...
    while child != null do
      if child.getElementType != TokenType.WHITE_SPACE then
        val bodyIndent = if simpleBody && child.getElementType != BendElements.Header &&
            child.getStartOffset > myNode.getStartOffset then Indent.getSpaceIndent(2, true)
  ...
  override def getSpacing(left: Block, right: Block): Spacing =
    ...
    if gap.exists(c => c == '\n' || c == '\r') then
      ...
      return Spacing.getReadOnlySpacing()
    if rightNode.getElementType == BendTokens.Separator && rightNode.getText == "," then
      Spacing.createSpacing(0, 0, 0, false, 0)
    else if leftNode.getElementType == BendTokens.Separator && leftNode.getText == "," &&
        !Set(")", "]", "}").contains(rightNode.getText) then
...
private object BendFormatSafety:
  def singleBodyLine(node: ASTNode, source: String): Boolean =
    if node.getElementType != BendElements.Definition && node.getElementType != BendElements.Law then
      return false
    var child = node.getFirstChildNode
    while child != null && child.getElementType != BendElements.Header do child = child.getTreeNext
    if child == null then return false
    val body = source.substring(child.getTextRange.getEndOffset, node.getTextRange.getEndOffset)
  ...
  def headersComplete(node: ASTNode): Boolean =
    if node.getElementType == BendElements.Header && !node.getText.trim.endsWith(":") then false
    else
      var child = node.getFirstChildNode
      var valid = true
  ...
  def complete(source: String): Boolean =
    ...
    while lexer.getTokenType != null do
      ...
      if kind == TokenType.BAD_CHARACTER || kind == BendTokens.InvalidEscape then valid = false
      val opening =
