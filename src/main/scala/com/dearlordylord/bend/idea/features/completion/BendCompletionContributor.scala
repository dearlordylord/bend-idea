  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters]:
      override def addCompletions(
        ...
        if !parameters.getOriginalFile.getLanguage
            .isKindOf(com.dearlordylord.bend.idea.syntax.BendLanguage.instance)
        then return
        ...
        BendCompletionContributor.context(source, offset).foreach { site =>
          val words = site match
            case BendCompletionContributor.Site.TopLevel =>
              BendCompletionContributor.declarations
            case BendCompletionContributor.Site.Body =>
              BendCompletionContributor.expressions
            case BendCompletionContributor.Site.Do =>
              BendCompletionContributor.doWords
            case BendCompletionContributor.Site.Match =>
              BendCompletionContributor.matchWords
            case BendCompletionContributor.Site.Law =>
              BendCompletionContributor.lawWords
            case BendCompletionContributor.Site.TypeHeader =>
              BendCompletionContributor.typeHeaderWords
            case BendCompletionContributor.Site.Type =>
              BendCompletionContributor.typeWords
            case BendCompletionContributor.Site.Qualified => Nil
          words.foreach { word =>
            result.addElement(
              LookupElementBuilder.create(word).withTypeText("Bend 2 keyword")
            )
          }
          ...
          val selected = ApplicationManager.getApplication
            ...
            .selection
          val (graph, visibleCurrent, visibleImported) =
          ...
          visibleCurrent
            ...
            .foreach { symbol =>
              val signature =
                symbol.signature.source.replaceAll("\\s+", " ").trim
              val comments = symbol.comments.replaceAll("\\s+", " ").trim
              val tail = "  " + signature + (if comments.isEmpty then ""
                                             else "  # " + comments)
              result.addElement(
                LookupElementBuilder
                  .create(symbol.declaration, symbol.name)
                  .withTailText(tail, true)
                  .withTypeText(symbol.category.toString.toLowerCase)
              )
            }
          if qualified.isEmpty && site != BendCompletionContributor.Site.TopLevel
          then
            graph.edges
              .filter(_.from == graph.root)
              .flatMap(_.importLine.alias)
              .distinct
              .filterNot(boundNames)
              .foreach { alias =>
                result.addElement(
                  LookupElementBuilder
                    .create(alias)
                    .withTypeText("import alias")
                )
              }
          visibleImported.foreach { case (name, symbol) =>
            if !boundNames.contains(name) then
              BendCompletionContributor.addSymbol(
                result,
                qualified,
                name,
                symbol,
                if graph.files.exists(f =>
                    f.source.id == symbol.handle.file && f.namespace.isEmpty
                  )
                then "Base "
                else "import ",
          ...
          val missingBase = graph.problems
            .collectFirst {
              case BendGraphProblem.Missing(_, line, path)
                  if line.spelling == "Base" =>
                path
            }
            .orElse {
              parameters.getOriginalFile.getProject
          ...
          missingBase match
            case Some(path)
                if Set(
                  BendCompletionContributor.Site.Body,
                  BendCompletionContributor.Site.Do,
                  BendCompletionContributor.Site.Match,
                  BendCompletionContributor.Site.Qualified
                ).contains(site) =>
              result
                .withPrefixMatcher("")
                .addElement(
                  LookupElementBuilder
                    .create("Configure Bend Base…")
                    .withTailText("  Base source unavailable: " + path, true)
                    .withTypeText("Bend setup")
                    .withInsertHandler(
                      (insertion: InsertionContext, _: LookupElement) =>
                        insertion.getDocument.deleteString(
                          insertion.getStartOffset,
                          insertion.getTailOffset
                        )
                        ShowSettingsUtil
                          .getInstance()
                          .showSettingsDialog(insertion.getProject, "Bend")
                    )
                )
            case _ => ()
          ...
          val triggers =
            site match
              case BendCompletionContributor.Site.TopLevel =>
                Set("def", "type", "law", "import")
              case BendCompletionContributor.Site.Body |
                  BendCompletionContributor.Site.Do |
                  BendCompletionContributor.Site.Match =>
                Set("match", "do")
              case _ => Set.empty[String]
          BendSnippets.all.filter(s => triggers.contains(s.trigger)).foreach {
            snippet =>
              result.addElement(
                LookupElementBuilder
                  .create(snippet.trigger)
                    ...
                    (insertion: InsertionContext, _: LookupElement) =>
                      ...
                      insertion.setLaterRunnable(() =>
                        TemplateManager
                          ...
                          .startTemplate(
                            editor,
                            BendSnippets.template(insertion.getProject, snippet)
                          )
                      )
                  )
              )
...
object BendCompletionContributor:
  ...
  private def addSymbol(
    ...
    val trailing = if qualified.nonEmpty then
      afterCaret
        .dropWhile(c => c.isLetterOrDigit || c == '_')
        .takeWhile(c => c.isLetterOrDigit || c == '_' || c == '.')
    else ""
    val signature = symbol.signature.source.replaceAll("\\s+", " ").trim
    val comments = symbol.comments.replaceAll("\\s+", " ").trim
    val tail =
      "  " + signature + (if comments.isEmpty then "" else "  # " + comments)
    val builder = LookupElementBuilder
      .create(symbol.declaration, lookup)
      ...
      .withInsertHandler((insertion: InsertionContext, _: LookupElement) =>
        ...
        while suffixEnd < document.getTextLength &&
          (document.getCharsSequence.charAt(suffixEnd).isLetterOrDigit ||
            document.getCharsSequence.charAt(suffixEnd) == '_')
        do suffixEnd += 1
        if suffixEnd > start + lookup.length then
          document.deleteString(start + lookup.length, suffixEnd)
        if trailing.startsWith(".") && !document.getCharsSequence
            .subSequence(start + lookup.length, document.getTextLength)
            .toString
            .startsWith(trailing)
        then document.insertString(start + lookup.length, trailing)
  ...
  private def qualifiedMember(
      source: String,
      offset: Int
  ): Option[(String, String)] =
    val before =
      source.substring(source.lastIndexOf('\n', offset - 1) + 1, offset)
    "([A-Za-z_][A-Za-z0-9_.]*)\\.([A-Za-z0-9_]*)$".r
      .findFirstMatchIn(before)
      .map(m => (m.group(1), m.group(2)))

  private def context(source: String, offset: Int): Option[Site] =
    if offset < 0 || offset > source.length || inNonCode(source, offset) then
      None
    else
      ...
      if before.matches(
          "\\s*(?:for|exs)\\s+[A-Za-z_][A-Za-z0-9_.]*\\s*:\\s*[A-Za-z_]*"
        )
      then Some(Site.Type)
      else if before.matches(".*[({][^)}]*:\\s*[A-Za-z_]*") then Some(Site.Type)
      else if before.matches("\\s*@unsafe\\s+[A-Za-z_]*") then
        Some(Site.TopLevel)
      else if before.matches("\\s*type\\s+[A-Za-z_][A-Za-z0-9_.]*\\s+is\\s+.*")
      then Some(Site.Type)
      else if before.matches("\\s*type\\s+[A-Za-z_][A-Za-z0-9_.]*\\s+") then
        Some(Site.TypeHeader)
      else if before.matches("\\s*(?:@unsafe\\s+)?def\\s+.*->\\s*[A-Za-z_]*")
      then Some(Site.Type)
      else if before.matches(
          "\\s*(?:@unsafe\\s+)?(?:def|type|law|import)\\s+.*"
        )
      then None
      ...
      else if before.forall(c =>
          c == ' ' || c == '\t' || c.isLetterOrDigit || c == '_'
        ) && before.takeWhile(c => c == ' ' || c == '\t').isEmpty
      then Some(Site.TopLevel)
      else if before.trim.isEmpty || before.matches(
          "\\s+[A-Za-z_][A-Za-z0-9_]*"
        ) || before.contains(':') || before.contains('=') || before.contains(
          '('
        )
      then
        val preceding = source
        ...
        val owner = preceding
          .find(line =>
            line.takeWhile(c => c == ' ' || c == '\t').length < indent
          )
          .getOrElse("")
        if previous.trim.startsWith("law ") || previous.trim.startsWith(
            "for "
          ) || previous.trim.startsWith("exs ")
        then Some(Site.Law)
        else if owner.trim.startsWith("type ") then None
        else if previous.trim.startsWith("match ") || previous.trim.startsWith(
            "case "
          )
        then Some(Site.Match)
  ...
  private def inNonCode(source: String, offset: Int): Boolean =
    ...
    while lexer.getTokenType != null && lexer.getTokenStart < offset do
      ...
      if token == BendTokens.Comment && offset <= lexer.getTokenEnd then
        return true
      lexer.advance()
    ...
    else if lexer.getTokenType == BendTokens.StringContent || lexer.getTokenType == BendTokens.Escape || lexer.getTokenType == BendTokens.InvalidEscape
    then lexer.getTokenStart <= offset && offset < lexer.getTokenEnd
    else false
