...
import com.dearlordylord.bend.idea.workspace.api.{BendImportLines, BendImportPaths, BendWorkspacePaths}
import com.intellij.codeInsight.AutoPopupController
import com.intellij.codeInsight.completion.{CompletionParameters, CompletionResultSet, InsertionContext}
import com.intellij.codeInsight.lookup.{LookupElement, LookupElementBuilder}
import com.intellij.openapi.application.ApplicationManager

/** One-level, offline import path suggestions. The catalog owns filesystem access. */
private[completion] object BendImportPathCompletion:
  private final case class Site(pathStart: Int, prefix: String)

  def add(parameters: CompletionParameters, result: CompletionResultSet, source: String,
      offset: Int): Boolean =
    site(source, offset) match
      ...
      case Some(location) =>
        ...
        val catalog = parameters.getOriginalFile.getProject.getService(classOf[BendWorkspacePaths])
        val directory = BendImportPaths.directory(file.getPath, selected.packageCache,
          location.prefix)
        val slash = location.prefix.lastIndexOf('/')
        val parent = if slash < 0 then "" else location.prefix.take(slash + 1)
        val names = scala.collection.mutable.LinkedHashMap.empty[String, Boolean]
        ...
        catalog.children(directory, 256).foreach { entry =>
          val name = entry.name
          val valid = name.nonEmpty && !name.startsWith(".") && name != "node_modules" &&
            !name.exists(c => c.isWhitespace || c == '#' || c == '\'' || c == '"')
          if valid then
            if entry.directory && !BendImportPaths.isHashRootFragment(parent + name) then
        ...
        names.iterator.filter { case (name, _) => name.startsWith(location.prefix) }
          .foreach { case (name, folder) =>
            result.withPrefixMatcher(location.prefix).addElement(
              LookupElementBuilder.create(name)
                .withTypeText(if name == "Base" then "Bend standard library"
                  else if folder then "Bend directory" else "Bend module")
                .withInsertHandler((insertion: InsertionContext, _: LookupElement) =>
                  ...
                  while end < document.getTextLength &&
                      ...
                      document.getCharsSequence.charAt(end) != '#' do end += 1
                  document.replaceString(location.pathStart, end, name)
                  insertion.getEditor.getCaretModel.moveToOffset(location.pathStart + name.length)
                  if folder then insertion.setLaterRunnable(() =>
  ...
  private def site(source: String, offset: Int): Option[Site] =
    if offset < 0 || offset > source.length then None
      ...
      line match
        case Import(head, path, tail) if
            offset >= start + head.length && offset <= start + head.length + path.length &&
            tail.matches("(?:[ \\t]+as[ \\t]+[A-Za-z_][A-Za-z0-9_]*)?[ \\t]*(?:#.*)?") &&
            BendImportLines.parse(source).exists(_.offset == start + head.indexOf("import")) =>
          Some(Site(start + head.length, source.substring(start + head.length, offset)))
        case _ => None
