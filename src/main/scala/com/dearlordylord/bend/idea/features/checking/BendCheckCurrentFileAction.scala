...
final class BendCheckCurrentFileAction
  ...
  override def actionPerformed(event: AnActionEvent): Unit =
    ...
    ProgressManager
      .getInstance()
      .run(new Task.Backgroundable(project, "Checking Bend", true):
        override def onCancel(): Unit = service.cancel(id)
        override def run(indicator: ProgressIndicator): Unit =
          val started = service.begin(
            initial,
            callback => {
          ...
          val checked = service
            .check(snapshot, () => indicator.isCanceled || project.isDisposed)
          ApplicationManager.getApplication.invokeLater(() => {
            if !project.isDisposed && document.getModificationStamp == initial.sourceRevision
            then
