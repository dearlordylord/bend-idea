...
/** Proof binders from the shared token stream. The policy is tolerant of
  * unfinished terms.
  */
private[scope] object BendProofScope:
  final case class LawClause(
      name: String,
      nameOffset: Int,
  ...
  def lawClauses(source: String, tokens: Vector[ScopeToken]): List[LawClause] =
    clauses(source, tokens).map { case (name, colon, boundary, _) =>
      ...
      LawClause(
        name.text,
        name.start,
        source.substring(tokens(colon).end, stop).trim
      )
    }
  ...
  def bindings(
      ...
      lawClauses.foreach { case (name, colon, boundary, prefix) =>
        if name.text != "_" then
          val where = topLevel(
            tokens,
            colon + 1,
            tokens.lift(boundary).map(_.start).getOrElse(end),
            "where"
          )
          val from = where
            .map(i => tokens(i).end)
            .orElse(tokens.lift(boundary).map(_.start))
            .getOrElse(end)
          out += ScopeBinding(
            name.text,
            name.start,
    ...
    for i <- tokens.indices do
      if Set("@", "@+", "@-", "&").contains(tokens(i).text) &&
        !tokens.lift(i - 1).exists(t => t.name && t.end == tokens(i).start)
      then
        val nameAt = (i + 1 until math.min(tokens.size, i + 4)).find(j =>
          tokens(j).name && tokens
            .slice(i + 1, j)
            .forall(t => Set("+", "-", "~").contains(t.text))
        )
        nameAt.foreach { j =>
          if tokens.lift(j + 1).exists(_.text == ":") then
            val arrow = topLevel(tokens, j + 2, closes(i), "->")
            arrow.foreach { k =>
              val clauseCap = lawClauses
                .find { case (_, colon, boundary, _) =>
                  colon < i && i < boundary
                }
                .map { case (_, colon, boundary, _) =>
                  ...
                  topLevel(tokens, colon + 1, boundaryOffset, "where")
                    .filter(_ > i)
                    .map(j => tokens(j).start)
                    .getOrElse(boundaryOffset)
                }
                .getOrElse(end)
              val cap = if tokens(i).start < headerEnd then
                math.min(headerEnd, closes(i))
              else math.min(closes(i), clauseCap)
              val prefix =
                if j > 0 && Set("+", "-", "~").contains(tokens(j - 1).text) then
                  tokens(j - 1).start
                else tokens(j).start
              out += ScopeBinding(
                tokens(j).text,
                tokens(j).start,
                source.substring(prefix, tokens(j).end),
                BindingOrigin.Parameter,
                tokens(k).end,
                cap
              )
      ...
      if tokens(i).text == "%" then
        topLevel(tokens, i + 1, closes(i), ":").foreach { colon =>
          ...
          val nextLine = ((colon + 1) until tokens.size).find(j =>
            tokens(j).start < closes(i) && line(source, tokens(j).start) > line(
              source,
              tokens(colon).start
            ) &&
          ...
          val cap = List(
            Some(closes(i)),
            semi.map(j => tokens(j).start),
            nextLine.map(j => tokens(j).start)
          ).flatten.min
          val named = tokens
            .lift(i + 1)
            .filter(_.name)
            .filter(_ => tokens.lift(i + 2).exists(_.text == "@"))
          named.foreach { name =>
            out += ScopeBinding(
              name.text,
              name.start,
          ...
          if tokens.exists(t =>
              t.text == "_" && t.start >= tokens(colon).end && t.start < cap
            )
          then
            out += ScopeBinding(
              "_",
