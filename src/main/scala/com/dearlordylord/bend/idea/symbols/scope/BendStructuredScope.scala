...
/** Case and do continuation regions, computed from the same source tokens as
  * ordinary locals. Columns are measured from the original text: a nested match
  * owns its own rows and a do continuation accepts another statement only at
  * the first statement's column or after `;`.
  */
private[scope] object BendStructuredScope:
  ...
  final case class Result(
    ...
    def regionEnd(offset: Int, fallback: Int): Int =
      regions
        .filter(r => r.from <= offset && offset < r.until)
        .map(_.until)
        .minOption
        .getOrElse(fallback)
  ...
  def inspect(source: String, tokens: Vector[ScopeToken], end: Int): Result =
    ...
    for i <- tokens.indices if owners(i) >= 0 do
      ...
      colon.foreach { headEnd =>
        ...
        val until =
          math.min(closes(i), nextCase.map(j => tokens(j).start).getOrElse(end))
        ...
        if from < until then
          ...
          pattern.indices.foreach { p =>
            ...
            if token.name && token.text != "_" && !constructor then
              val prefix =
                if p > 0 && pattern(p - 1).text == "+" &&
                  pattern(p - 1).end == token.start
                then pattern(p - 1).start
                else token.start
              bindings += ScopeBinding(
                token.text,
                token.start,
                source.substring(prefix, token.end),
    ...
    for i <- tokens.indices if tokens(i).text == "do" do
      headingColon(tokens, i + 1).foreach { colon =>
        ...
        if first < tokens.size then
          ...
          val caseCap = caseRegions
            .filter(r =>
              r.from <= tokens(i).start &&
                tokens(i).start < r.until
            )
            .map(_.until)
            .minOption
            .getOrElse(end)
          ...
          if tokens(first).start < cap then
            ...
            while start < tokens.size && tokens(start).start < cap && !done do
              var cursor = start + 1
              var depth =
                if Set("(", "[", "{").contains(tokens(start).text) then 1 else 0
              var separator = -1
              ...
              while cursor < tokens.size && tokens(
                  cursor
                ).start < cap && separator < 0
              do
                val token = tokens(cursor)
                val prefix = tokens.slice(start, cursor)
                val assignment = if typedStart then
                  prefix.indexWhere(t => t.text == "=" || t.text == "<-")
                else -1
                val rhsStarted =
                  !typedStart || assignment >= 0 && assignment + 1 < prefix.size
                if depth == 0 && (token.text == ";" ||
                    newLine(source, tokens(cursor - 1), token) && rhsStarted)
                then separator = cursor
                else
                  token.text match
                    case "(" | "[" | "{" => depth += 1
              ...
              val next = if separator < 0 then cursor
              else if tokens(separator).text == ";" then separator + 1
              else separator
              val statement = tokens.slice(start, stop)
              val bind = doAssignment(statement)
              bind.foreach { case (relative, _) =>
                assignments += start + relative
              }
              val hasNext = next < tokens.size && tokens(next).start < cap
              val hasRhs = bind.exists { case (relative, _) =>
                relative + 1 < statement.size
              }
              if hasNext && hasRhs then
                bind
                  .filter { case (relative, _) =>
                    relative + 1 < statement.size
                  }
                  .foreach { case (_, name) =>
                    val prefix =
                      if statement.head.text == "+" || statement.head.text == "-"
                      then statement.head.start
                      else name.start
                    pending += ((name, prefix, tokens(next).start))
                  }
              val step = separator >= 0 && (tokens(separator).text == ";" ||
                col(source, tokens(separator).start) == column)
              if hasNext && (hasRhs || bind.isEmpty && statement.headOption
                  .exists(_.text != "return") && step)
              then start = next
              else
            ...
            pending.result().foreach { case (name, prefix, from) =>
              if from < until then
                bindings += ScopeBinding(
                  name.text,
                  name.start,
                  source.substring(prefix, name.end),
                  BindingOrigin.Do,
                  from,
                  until
                )
    ...
    for
      i <- tokens.indices if (tokens(i).text == "Array.set" ||
        (tokens(i).name && i + 1 < tokens.size && tokens(i + 1).text == "[")) &&
        standaloneTermStart(source, tokens, i) &&
        !allRegions.exists(r =>
          r.doBlock && r.from <= tokens(i).start && tokens(i).start < r.until
        )
    do
      ...
      var cursor = i + 1
      var boundary = -1
      while cursor < tokens.size && tokens(cursor).start < closes(
          i
        ) && boundary < 0
      do
        val token = tokens(cursor)
        if depth == 0 && (token.text == ";" ||
            (newLine(source, tokens(cursor - 1), token) && col(
              source,
              token.start
            ) == column))
        then boundary = cursor
        else
          token.text match
            case "(" | "[" | "{" => depth += 1
            case ")" | "]" | "}" => depth = math.max(0, depth - 1)
      ...
      if boundary >= 0 then
        val next =
          if tokens(boundary).text == ";" then boundary + 1 else boundary
        if next < tokens.size then
          arrayWriteBinder(tokens.slice(i, boundary)).foreach { name =>
            val until = math.min(
              ...
              allRegions
                .filter(r =>
                  r.from <= tokens(i).start && tokens(i).start < r.until
                )
                .map(_.until)
                .minOption
                .getOrElse(end)
  ...
  /** A match accepts its first row only when it is deeper than the surrounding
    * case. Later rows may be at any column at least as deep as that first row.
    * When a row returns to an outer column, the innermost match no longer owns
    * it.
    */
  private def caseOwners(
      source: String,
      tokens: Vector[ScopeToken]
      ...
      if tokens(i).text == "match" then
        headingColon(tokens, i + 1).foreach { colon =>
          ...
          if first < tokens.size && tokens(first).text == "case" &&
            col(source, tokens(first).start) > parentColumn
          then
            stack.lastOption.foreach(parent => parents(i) = parent.id)
            stack += MatchFrame(
              i,
              col(source, tokens(first).start),
              parentColumn
            )
        }
  ...
  private def doAssignment(
      statement: Vector[ScopeToken]
  ): Option[(Int, ScopeToken)] =
    val nameAt =
      if statement.headOption.exists(t => t.text == "+" || t.text == "-") then 1
      else 0
    if nameAt >= statement.size || !statement(nameAt).name ||
      nameAt + 1 >= statement.size || statement(nameAt + 1).text != ":"
    then None
    else
      var depth = 0
      statement.indices
  ...
  private def arrayWriteBinder(
      statement: Vector[ScopeToken]
  ): Option[ScopeToken] =
    if statement.headOption
        .exists(_.name) && statement.lift(1).exists(_.text == "[")
    then
      val close = statement.indexWhere(_.text == "]", 2)
      if close > 2 && statement.lift(close + 1).exists(_.text == "<-") &&
        statement.lift(close + 2).nonEmpty
      then statement.headOption
      else None
      ...
      if open < 0 then None
        ...
        if firstComma >= 0 && secondComma == firstComma + 2 &&
          statement(firstComma + 1).name
        then Some(statement(firstComma + 1))
        else None

  private def standaloneTermStart(
      source: String,
      tokens: Vector[ScopeToken],
      at: Int
  ): Boolean =
    if at == 0 then true
      ...
      previous.text == ";" ||
      (previous.text == ":" && tokens.indices
        .take(at - 1)
        .exists(j =>
          tokens(j).text == "case" && headingColon(tokens, j + 1)
            .contains(at - 1)
        )) ||
    ...
    for i <- tokens.indices do
      tokens(i).text match
        ...
        case ")" | "]" | "}" if stack.nonEmpty =>
          ...
          for j <- open + 1 until i do
            limits(j) = math.min(limits(j), tokens(i).start)
        case _ => ()
    limits
