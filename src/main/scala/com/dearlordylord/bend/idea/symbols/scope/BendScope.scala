...
/** Lexer-derived source token. Offsets are UTF-16 offsets in the current
  * document.
  */
...
/** A source binder is eligible only in [from, until). Its spelling retains
  * quantity/template syntax.
  */
final case class ScopeBinding(
    name: String,
    nameOffset: Int,
    source: String,
...
/** Pure, tolerant lexical scope policy over the shared lexer's token
  * projection.
  */
object BendScope:
  def lawClauses(
  ...
  def bindings(
    ...
    parameterBindings(source, header, limit) ++
      ...
      BendProofScope.bindings(
        source,
        tokens.filter(_.start < limit),
        headerEnd,
        limit,
        law
      )

  /** The surface parser deliberately retains malformed neighbors; a fresh
    * column-zero line outside delimiters is no longer in this declaration's
    * body.
    */
  private def bodyLimit(
      source: String,
      body: Vector[ScopeToken],
      headerEnd: Int,
      declarationEnd: Int
  ): Int =
    var depth = 0
    ...
    while i < body.size do
      ...
      if depth == 0 && source.substring(previous, token.start).contains('\n') &&
        lineIndent(source, token.start) == 0
      then return token.start
      token.text match
        case "(" | "[" | "{" => depth += 1
  ...
  def visible(bindings: List[ScopeBinding], offset: Int): List[ScopeBinding] =
    bindings
      .filter(b => b.from <= offset && offset < b.until)
    ...
    if open < 0 then Nil
      ...
      sections.flatMap { case (first, last) =>
        ...
        binder.filter(_.text != "_").map { name =>
          val from = if last < tokens.size then tokens(last).end
          else tokens.lastOption.map(_.end).getOrElse(until)
          ScopeBinding(
            name.text,
            name.start,
            source.substring(part.head.start, part.last.end),
  ...
  private def bodyBindings(
      source: String,
      tokens: Vector[ScopeToken],
      end: Int,
      structured: BendStructuredScope.Result
  ): List[ScopeBinding] =
      ...
      tokens(i).text match
        ...
        case ")" | "]" | "}" if stack.nonEmpty =>
          ...
          for j <- open + 1 until i do
            enclosing(j) = math.min(enclosing(j), tokens(i).start + 1)
        case _ => ()
    ...
    for i <- tokens.indices do
      if tokens(i).text == "=>" && i > 0 && tokens(i - 1).name then
        val name = tokens(i - 1)
        val quantity =
          if i > 1 && Set("+", "-").contains(tokens(i - 2).text) &&
            tokens(i - 2).end == name.start
          then tokens(i - 2).start
          else name.start
        val stop = expressionEnd(source, tokens, i + 1, enclosing(i), end)
        out += ScopeBinding(
          name.text,
          name.start,
          source.substring(quantity, name.end),
          BindingOrigin.Lambda,
          tokens(i).end,
          stop
        )
      if tokens(i).text == "=" && !structured.consumedAssignments.contains(i)
      then
        ...
        val valid = binders.nonEmpty && left.nonEmpty &&
          (colon >= 0 || left.forall(t =>
            t.name || Set("-", "+", "~").contains(t.text)
          ))
        if valid then
          val boundary = statementEnd(source, tokens, i + 1, enclosing(i), end)
          val from = boundary._1
          val stop =
            math.min(enclosing(i), structured.regionEnd(tokens(i).start, end))
          binders.foreach { name =>
            val previous = left.indexWhere(_.start == name.start) - 1
            val start =
              if previous >= 0 && Set("-", "+", "~").contains(
                  left(previous).text
                ) &&
                left(previous).end == name.start
              then left(previous).start
              else name.start
            out += ScopeBinding(
              name.text,
              name.start,
              source.substring(start, name.end),
              BindingOrigin.Let,
              from,
              stop
            )
    ...
    while i < tokens.size && tokens(i).start < enclosingEnd do
      ...
      if depth == 0 then
        ...
        if i > from && source
            ...
            .contains('\n') &&
          lineIndent(source, token.start) <= indent
        then return (token.start, i)
      token.text match
        case "(" | "[" | "{" => depth += 1
        case ")" | "]" | "}" => depth -= 1
        case _               => ()
  ...
  private def splitAtCommas(
    ...
    for i <- first until last do
      tokens(i).text match
        case "(" | "[" | "{"                                      => depth += 1
        case ")" | "]" | "}"                                      => depth -= 1
        case op if op.startsWith("<") && op != "<=" && op != "<-" =>
          depth += op.takeWhile(_ == '<').length
