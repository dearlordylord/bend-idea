...
final case class BendDocumentationSite(symbol: BendSourceSymbol,
    law: Option[BendSourceSymbol], fills: List[BendSourceSymbol]):
...
object BendSourceDocumentation:
  ...
  def site(requestFile: PsiFile, symbol: BendSourceSymbol): BendDocumentationSite =
    val project = requestFile.getProject
    val (basePath, packageCache) = project.getService(classOf[BendLoadingConfiguration]).paths
    val graph = project.getService(classOf[BendImportedSymbolCatalog])
      .loaded(requestFile, basePath, packageCache)
    val rootDeclarations = BendSourceSymbols.declarations(requestFile)
    val aliases = project.getService(classOf[BendImportedSymbolCatalog]).effectiveDirectEdges(graph)
      .flatMap(edge => edge.target.toList.flatMap(target =>
    ...
    val laws = loaded.collect { case (id, law) if law.category == BendSymbolCategory.Law =>
      (id, law)
    }
    val definitions = loaded.collect { case (id, fill) if fill.category == BendSymbolCategory.Definition =>
      (id, fill)
    }
    val links = laws.flatMap { case (id, law) =>
      val names = if id == graph.root || relevant.exists(f =>
          f.source.id == id && f.namespace.isEmpty) then List(law.name)
        else aliases.collect { case (`id`, alias) => alias + "." + law.name }
      val matches = definitions.collect { case (fillId, fill)
          if (fillId == id || fillId == graph.root) &&
              (if fillId == id then List(law.name) else names).exists { name =>
            ...
            BendLawDeclarations.isFill(BendSourceSymbols.site(qualifiedLaw),
              BendSourceSymbols.site(fill), requireOrder = fillId == id)
          } => fill
      }.distinctBy(_.handle)
    ...
    BendDocumentationSite(symbol,
      paired.flatMap { case (law, _) => Option.when(law.handle != symbol.handle)(law) },
