...
/** The source symbol owner consumes one workspace loading policy for completion and later references. */
final class BendImportedSymbolCatalog(project: Project):
  private var parsed = Map.empty[FileId, (String, List[BendSourceSymbol])]

  def loaded(file: PsiFile, basePath: String, packageCache: String): BendLoadedGraph =
    val id = BendSourceSymbols.fileId(file)
    val path = Option(file.getVirtualFile).map(_.getPath)
      .orElse(Option(file.getOriginalFile.getVirtualFile).map(_.getPath))
      .getOrElse(file.getName)
    val text = file.getText
    val root = BendSourceRecord(id, path, text, file.getModificationStamp,
      BendImportLines.parse(text))
    project.getService(classOf[BendWorkspaceGraph]).load(root, basePath, packageCache)

  def visible(file: PsiFile, basePath: String, packageCache: String):
  ...
  /** Alias expansion wins when the imported target exists; otherwise a literal
    ...
    */
  def visibleWithCurrent(file: PsiFile, offset: Int, basePath: String,
      packageCache: String): (BendLoadedGraph, List[BendSourceSymbol],
      List[(String, BendSourceSymbol)]) =
    val graph = loaded(file, basePath, packageCache)
    val direct = directDeclarations(graph)
    val expanded = direct.iterator.map(_._1).toSet
    val current = BendSourceSymbols.visibleCandidates(file, offset)
      .filterNot(symbol => expanded.contains(symbol.name))
    val currentKeys = current.map(symbol => (symbol.name, symbol.category)).toSet
  ...
  def resolve(file: PsiFile, offset: Int, spelling: String, basePath: String,
      packageCache: String, category: Option[BendSymbolCategory] = None): BendSourceResolution =
    val (_, current, imported) = visibleWithCurrent(file, offset, basePath, packageCache)
    ...
    binding match
      ...
      case None =>
        val matches = current.filter(s => s.name == spelling && category.forall(_ == s.category)) ++
          imported.collect { case (`spelling`, s) if category.forall(_ == s.category) => s }
        matches match
  ...
  private def declarations(source: BendSourceRecord): List[BendSourceSymbol] = synchronized {
    parsed.get(source.id) match
      ...
      case _ =>
        val symbols = BendSourceSymbols.loadedDeclarations(project, source.id, source.text)
