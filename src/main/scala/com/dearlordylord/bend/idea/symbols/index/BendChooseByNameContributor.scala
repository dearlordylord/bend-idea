...
final class BendChooseByNameContributor extends ChooseByNameContributorEx:
  override def processNames(processor: Processor[? >: String], scope: GlobalSearchScope,
    ...
    if !DumbService.getInstance(project).isDumb then
      try
        continue = StubIndex.getInstance.processAllKeys(BendSymbolNameIndex.Key,
          new Processor[String]:
            override def process(name: String): Boolean =
              ...
              StubIndex.getInstance.processElements(BendSymbolNameIndex.Key, name, project,
                searchScope, indexFilter, classOf[BendDeclaration],
  ...
  override def processElementsWithName(name: String,
    ...
    if !DumbService.getInstance(project).isDumb then
      try
        continue = StubIndex.getInstance.processElements(BendSymbolNameIndex.Key, name,
          project, scope, indexFilter, classOf[BendDeclaration], new Processor[BendDeclaration]:
            override def process(declaration: BendDeclaration): Boolean = emit(declaration))
      catch
        case _: IndexNotReadyException => ()
    if continue then
  ...
  private def excludingOpenFiles(filter: IdFilter, openFiles: List[PsiFile]): IdFilter =
    val openFileIds = openFiles.flatMap(file => Option(file.getVirtualFile))
      .map(FileBasedIndex.getFileId).toSet
    if openFileIds.isEmpty then filter
    else new IdFilter:
      override def containsFileId(fileId: Int): Boolean =
        !openFileIds.contains(fileId) && (filter == null || filter.containsFileId(fileId))

  private def isIncluded(file: PsiFile, scope: GlobalSearchScope, filter: IdFilter): Boolean =
    Option(file.getVirtualFile).exists { virtualFile =>
      scope.contains(virtualFile) &&
        (filter == null || filter.containsFileId(FileBasedIndex.getFileId(virtualFile)))
    }
