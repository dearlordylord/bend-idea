...
/** Bounded, current-document-aware source discovery shared by usage search and
  * rename.
  */
...
object BendSourceFiles:
  ...
  def files(origin: PsiFile, scope: SearchScope): List[PsiFile] =
    scan(origin, scope).files

  def scan(origin: PsiFile, scope: SearchScope): BendSourceFileScan =
    scan(origin, scope, includeConfiguredLibraries = false)

  /** Rename may need configured library sources even when IntelliJ has not
    * indexed them.
    */
  def scan(
      origin: PsiFile,
      scope: SearchScope,
      includeConfiguredLibraries: Boolean
  ): BendSourceFileScan =
    val project = origin.getProject
    scope match
      case local: LocalSearchScope =>
        val candidates = local.getScope.iterator
          .map(_.getContainingFile)
          .filter(_ != null)
          .filter(_.getLanguage == BendLanguage.instance)
          .toList
          .distinct
        return BendSourceFileScan(
          candidates.filter(_.getTextLength <= MaxText).take(MaxFiles),
          candidates.size <= MaxFiles && candidates.forall(
            _.getTextLength <= MaxText
          )
        )
      case _ => ()
    ...
    val libraries =
      List(base, cache).flatMap(path => Option(local.findFileByPath(path)))
    libraries
      .filter(file => includeConfiguredLibraries || inScope(scope, file))
      .foreach(enqueue)
    def selected(file: VirtualFile): Boolean =
      inScope(scope, file) || includeConfiguredLibraries && libraries.exists {
        root =>
          file == root || root.isDirectory && file.getPath.startsWith(
            root.getPath.stripSuffix("/") + "/"
          )
      }
    ...
    while queue.nonEmpty && visited < MaxVisited && accepted.size < MaxFiles do
      ...
      if seen.add(next) && next.isValid then
        visited += 1
        if next.isDirectory then next.getChildren.foreach(enqueue)
        else if next.getName.endsWith(".bend") && selected(next) then
          val document = documents.getCachedDocument(next)
          val length = if document == null then next.getLength
          else document.getTextLength.toLong
          if length > MaxText then complete = false
          else
            Option(manager.findFile(next))
              .filter(_.getLanguage == BendLanguage.instance)
              .filter(_.getTextLength <= MaxText)
              .foreach(accepted += _)
    if queue.nonEmpty then complete = false
    if !accepted.contains(origin) && origin.getTextLength <= MaxText &&
      Option(origin.getVirtualFile).exists(inScope(scope, _))
    then accepted.prepend(origin)
    BendSourceFileScan(accepted.toList, complete)

  private def inScope(scope: SearchScope, file: VirtualFile): Boolean =
    scope match
