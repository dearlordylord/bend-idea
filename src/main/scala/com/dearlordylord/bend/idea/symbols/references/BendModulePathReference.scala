...
/** The file owns the complete path range even when the lexer splits its punctuation. */
final class BendModulePathReference(file: PsiFile, range: TextRange, importOffset: Int)
  ...
  override def resolve(): PsiElement =
    ...
    val graph = project.getService(classOf[BendImportedSymbolCatalog]).loaded(getElement, base, cache)
    graph.importTarget(graph.root, importOffset)
      .flatMap(BendPhysicalTargets.file(project, _)).orNull
