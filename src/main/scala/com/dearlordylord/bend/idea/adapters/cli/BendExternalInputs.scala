  ...
  def stamp(executable: String, basePath: Option[String]): String =
    def file(path: String, source: Boolean): String =
      try
        ...
        val attrs = Files.readAttributes(normalized, classOf[BasicFileAttributes])
    ...
    val observed = file(executable, false) + basePath.fold("")(path => ":" + file(path, true))
    com.dearlordylord.bend.idea.analysis.model.BendAnalysisKey.sourceDigest(observed)
