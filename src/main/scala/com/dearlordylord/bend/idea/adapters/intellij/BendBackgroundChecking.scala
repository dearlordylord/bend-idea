...
/** Project-owned debounce and worker lifetime. Captures and processes run off
  * the UI thread.
  */
final class BendBackgroundChecking(project: Project) extends Disposable:
      ...
      new DocumentListener:
        override def documentChanged(event: DocumentEvent): Unit =
          ...
          if source != null && source.getName.endsWith(".bend") then
            ...
            synchronized { knownRoots.toList }
              .collect {
                case (root, file) if affected(root) => file
              }
              .foreach(schedule)
            if !synchronized { knownRoots.contains(sourceId) } then
              schedule(source)
      ,
      this
    )
  ...
  private def check(
    ...
    if disposed || project.isDisposed || !settings.selection.diagnosticsEnabled ||
      synchronized { tickets.get(root) != Some(ticket) }
    then return
    val selection = settings.selection
