...
import com.dearlordylord.bend.idea.analysis.api.{BendBackgroundCheckControl,
  BendBackgroundCheckTicket, BendCheckService, BendGraphSnapshot}
...
/** IntelliJ timer, source capture, and worker adapter for the immutable check policy. */
final class BendBackgroundChecking(project: Project) extends Disposable:
    ...
    override def run(): Unit =
      if !disposed && !project.isDisposed then
        synchronized { rootFiles.keys.toList }.foreach { root =>
          if service.result(root).exists(!_.fresh) then control.backgroundStaleObserved(root)
        }
  ...
  private def request(file: VirtualFile): Unit =
    if disposed || project.isDisposed || !settings.selection.diagnosticsEnabled ||
        !file.isValid || !file.getName.endsWith(".bend") then return
  ...
  EditorFactory.getInstance().getEventMulticaster.addDocumentListener(new DocumentListener:
    override def documentChanged(event: DocumentEvent): Unit =
      ...
      if source != null && source.getName.endsWith(".bend") then
        ...
        val candidates = synchronized { affected.flatMap(rootFiles.get).toList }
  ...
  project.getMessageBus.connect(this).subscribe(FileEditorManagerListener.FILE_EDITOR_MANAGER,
    new FileEditorManagerListener:
      override def fileOpened(source: FileEditorManager, file: VirtualFile): Unit = request(file)
  )
  ...
  private[intellij] def applyPolicyAction(action: BendCheckAction): Unit = action match
    case ScheduleBackground(root, token, delayMillis) =>
      val file = synchronized { rootFiles.get(root) }
      file.foreach { source =>
        if !disposed && !project.isDisposed then synchronized {
  ...
  private def check(file: VirtualFile, ticket: BendBackgroundCheckTicket): Unit =
    if disposed || project.isDisposed || !control.backgroundCurrent(ticket) then return
    val selection = settings.selection
    ...
    val captured = ReadAction.compute(() => {
      if !file.isValid then None
        ...
        (document, BendCheckSnapshot(ticket.root, file.getPath, document.getText,
    ...
    try
      val source = BendSourceRecord(ticket.root, initial.path, initial.text,
        initial.sourceRevision, BendImportLines.parse(initial.text))
  ...
  override def dispose(): Unit =
    ...
    if shouldNotify && !project.isDisposed then control.backgroundSchedulerDisposed()
