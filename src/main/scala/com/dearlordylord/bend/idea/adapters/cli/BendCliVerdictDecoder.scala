...
/** Classifies only complete, documented Bend CLI reports. Unknown text has no semantic facts. */
private[cli] object BendCliVerdictDecoder:
  ...
  private val ProofSiblingGuard = "bend: PROOF.bend must import ./LAWS.bend (see bend --help)"
  ...
  def capability(process: BendProcessOutcome): Capability = process match
    case BendProcessOutcome.Exited(0, output) =>
      ...
      if help.split("\n", -1).contains(CheckOnlyHelpLine) then Capability.Supported(help)
      else Capability.Unavailable("This Bend compiler does not document --check-only; no source was checked.")
    case BendProcessOutcome.Exited(_, _) =>
      Capability.Unavailable("This Bend compiler does not document --check-only; no source was checked.")
    case BendProcessOutcome.TimedOut(output) =>
      Capability.TimedOut(withOutput("Bend capability probe timed out; no source was checked.", output))
    case BendProcessOutcome.Canceled(_) =>
      Capability.Canceled("Bend capability probe canceled; no source was checked.")
    case BendProcessOutcome.OutputLimit(output) =>
      Capability.OutputLimit(withOutput("Bend capability probe output exceeded the limit; no source was checked.", output))
    case BendProcessOutcome.StartFailed(message) =>
      Capability.Unavailable("Could not start Bend capability probe: " + message + "; no source was checked.")
  ...
  def check(process: BendProcessOutcome): Check = process match
    case BendProcessOutcome.Exited(code, output) =>
      ...
      if code == 0 && report == "All terms check." then
        Check(BendCheckOutcome.Success, BendCompleteness.Complete, BendReliance.None, report)
      else if code == 0 then
        unsafeReliance(report) match
          case Some(reliance) =>
            Check(BendCheckOutcome.Success, BendCompleteness.Complete, reliance, report)
          case None =>
            Check(BendCheckOutcome.Unavailable, BendCompleteness.Unknown, BendReliance.Unknown,
              unrecognized(code, report))
      else if todoReport(report) then
        Check(BendCheckOutcome.Failed, BendCompleteness.Incomplete, BendReliance.Unknown, report)
      else if compilerErrorReport(report) then
        Check(BendCheckOutcome.Failed, BendCompleteness.Unknown, BendReliance.Unknown, report,
          sourceLocationAllowed = compilerLocationReport(report))
      else
        Check(BendCheckOutcome.Unavailable, BendCompleteness.Unknown, BendReliance.Unknown,
          unrecognized(code, report))
    case BendProcessOutcome.TimedOut(output) =>
      Check(BendCheckOutcome.TimedOut, BendCompleteness.Unknown, BendReliance.Unknown,
    ...
    case BendProcessOutcome.StartFailed(message) =>
      Check(BendCheckOutcome.Unavailable, BendCompleteness.Unknown, BendReliance.Unknown, message)

  private def normalize(output: String): String =
    output.replaceAll("\\u001b\\[[0-9;]*m", "").replace("\r\n", "\n").stripSuffix("\n")
  ...
  private def unsafeReliance(report: String): Option[BendReliance] =
    ...
    lines match
      case UnsafeHeader(countText, pluralDef, verb) :: definitions if
          countText.toIntOption.exists(_ > 0) && definitions.nonEmpty &&
            definitions.forall(line => UnsafeDefinition.matches(line)) =>
        val count = countText.toInt
        val grammarMatches = if count == 1 then pluralDef == "def" && verb == "relies"
          else pluralDef == "defs" && verb == "rely"
        Option.when(grammarMatches && definitions.size == count)(BendReliance.UnsafeOrForeign)
      case _ => None

  private def todoReport(report: String): Boolean =
    report.split("\n", -1).toList match
      case TodoHeader(countText, plural) :: "The code is incomplete, and not a valid proof yet." :: Nil =>
  ...
  private def compilerErrorReport(report: String): Boolean =
    ...
    report == ProofSiblingGuard || (lines.length >= 2 && lines(0) == "Error:" &&
      (lines(1).startsWith("- message  : ") || lines(1).startsWith("- expected : ")))
  ...
  private def compilerLocationReport(report: String): Boolean =
    ...
    def recognizedFields(fields: List[String]): Boolean =
      fields match
        case first :: rest if first.startsWith("- message  : ") || first.startsWith("- expected : ") =>
          rest.foldLeft(Option(false)) {
            case (Some(_), "Context:") => Some(true)
            case (Some(inContext), field) if recognizedField(field, inContext) => Some(inContext)
            case _ => None
          }.isDefined
        case _ => false
    val beforeLocation = lines.take(location).drop(1)
    val excerpts = lines.drop(location + 1)
    location >= 0 && location < lines.size - 1 && recognizedFields(beforeLocation) &&
  ...
  private def unrecognized(exitCode: Int, report: String): String =
    ...
    else s"Bend returned unrecognized failure output (exit code $exitCode); no compiler verdict was inferred."
