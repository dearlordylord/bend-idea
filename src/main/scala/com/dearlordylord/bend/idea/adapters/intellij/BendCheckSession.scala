...
final class BendCheckSession(project: Project) extends BendCheckService, BendBackgroundCheckControl,
  ...
  private def applyEvent(event: BendCheckEvent): BendCheckTransition = synchronized {
    val next = BendCheckTransitionPolicy.transition(policyState, event)
    policyState = next.state
    next
  }

  private def perform(actions: Seq[BendCheckAction],
      replacement: Option[(FileId, (() => Unit) => (() => Unit))] = None): Unit =
    actions.foreach {
      case ReplaceRootSubscription(root) =>
        replacement.filter(_._1 == root).foreach { case (_, subscribe) =>
          ...
          val previous = synchronized {
            if policyState.disposed || !policyState.roots.contains(root) then None
      ...
      case UnsubscribeRoot(root) =>
        ...
        previous.foreach(_())
      case CancelWorker(generation) => synchronized { canceledWorkers += generation }
  ...
  private def scheduleUiRefresh(): Unit =
    ...
    uiRefreshAlarm.addRequest(restartRequest, uiRefreshDelayMillis, ModalityState.any())
  ...
  private def refreshUiBeforeReturning(): Unit =
    ...
    if !disposed && !project.isDisposed then
      ...
      val restart = new Runnable:
        override def run(): Unit =
          if !project.isDisposed && !synchronized { policyState.disposed } then analyzer.restart()
  ...
  private def sourceChanged(root: FileId): Unit =
    val result = applyEvent(InputsInvalidated(Set(root), BendCheckInvalidationReason.SourceChanged))
    ...
    override def documentChanged(event: DocumentEvent): Unit =
      ...
      if file != null then
        ...
        val roots = synchronized {
          policyState.roots.toList.collect { case (root, rootState) if root != identity &&
            rootDependsOn(rootState, identity) => root
          }.toSet
        }
        if roots.nonEmpty then
          val result = applyEvent(InputsInvalidated(roots,
    ...
    new BulkFileListener:
      override def after(events: java.util.List[? <: VFileEvent]): Unit =
        ...
        val affected = synchronized {
          policyState.roots.toList.collect { case (root, rootState) if
            rootState.result.exists(result => changed(result.key.executable) ||
              result.key.basePath.exists(changed)) ||
              (rootState.snapshot.toList ++ rootState.pending.toList.map(_.snapshot)).exists(
                snapshotAffectedBy(_, changed)) => root
          }.toSet
        }
        if affected.nonEmpty then
          val result = applyEvent(InputsInvalidated(affected,
            BendCheckInvalidationReason.DependencyChanged))
          perform(result.actions :+ RefreshUi)
  )

  private def rootDependsOn(root: BendRootCheckState, source: FileId): Boolean =
    (root.snapshot.toList ++ root.pending.toList.map(_.snapshot)).exists { snapshot =>
      snapshot.root != source && (snapshot.graph.exists(_.files.exists(_.source.id == source)) ||
        snapshot.siblingLaws.exists(_.id == source))
    }

  private def snapshotAffectedBy(snapshot: BendCheckSnapshot, changed: Set[String]): Boolean =
    changed(snapshot.path) || snapshot.graph.exists(graph =>
      graph.files.exists(file => changed(file.source.path) || changed(file.source.id.value)) ||
        ...
        case WaitingForWorker(generation) =>
          ...
          val released = synchronized {
            while policyState.active.exists(_.generation == generation) &&
                !policyState.disposed && System.nanoTime() < deadline do wait(50L)
            !policyState.disposed && !policyState.active.exists(_.generation == generation)
          }
          if !released then
  ...
  override def result(root: FileId): Option[BendCheckResult] =
    val (stored, captured, generation) = synchronized {
      policyState.roots.get(root).map(rootState =>
        (rootState.result, rootState.snapshot, rootState.generation))
        .getOrElse((None, None, 0L))
    }
    val graphChanged = stored.exists(_.fresh) && captured.exists(snapshot => !graphCurrent(snapshot))
    val externalCurrent = stored.forall(result => result.key.externalStamp ==
    ...
    if stored.exists(_.fresh) && (graphChanged || !externalCurrent) then
      val reason = if graphChanged then BendCheckInvalidationReason.DependencyChanged
        else BendCheckInvalidationReason.ExternalInputChanged
      val invalidated = captured.map(snapshot => SnapshotInvalidated(root, generation, snapshot, reason))
  ...
  override def resultsFor(source: FileId): List[BendCheckResult] =
    ...
    synchronized { policyState.roots.values.flatMap(_.result).toList }
      .filter(checked => checked.key.root == source || checked.sources.exists(_.id == source))
