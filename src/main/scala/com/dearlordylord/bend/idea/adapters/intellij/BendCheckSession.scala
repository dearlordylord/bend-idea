...
final class BendCheckSession(project: Project)
        ...
        override def documentChanged(event: DocumentEvent): Unit =
          ...
          if file != null then
            ...
            val roots = synchronized {
              (captures.toList ++ pendingCaptures.toList).collect {
                case (root, snapshot)
                    if root != identity && (snapshot.graph
                      .exists(_.files.exists(_.source.id == identity)) ||
                      snapshot.siblingLaws.exists(_.id == identity)) =>
                  root
              }.distinct
            }
      ...
      new BulkFileListener:
        override def after(events: java.util.List[? <: VFileEvent]): Unit =
          ...
          val affected = synchronized {
            val roots = results.iterator.collect {
              case (root, result)
                  if changed(result.key.executable) ||
                    ...
                    captures
                      ...
                      .exists(graph =>
                        graph.files.exists(file =>
                          changed(file.source.path) ||
                            changed(file.source.id.value)
                        ) ||
                          graph.edges
                            .exists(edge => changed(edge.requestedPath))
                      ) ||
                    captures
                      .get(root)
                      .exists(snapshot =>
  ...
  private def sourceChanged(root: FileId): Unit = synchronized {
    if !disposed then
      ...
      results
        .get(root)
        .foreach(result => results(root) = result.copy(fresh = false))
      ApplicationManager.getApplication.invokeLater(() => {
        if !project.isDisposed then
          DaemonCodeAnalyzer.getInstance(project).restart()
      })
  ...
  override def begin(
    ...
    if !background && active.exists(_._4) then
      ...
      results
        .get(previous)
        .foreach(result => results(previous) = result.copy(fresh = false))
      if !started then active = None
        ...
        while active.nonEmpty && !disposed && System.nanoTime() < deadline do
          wait(50)
  ...
  override def check(
    ...
    reservation match
      ...
      case Some((_, generation, isCurrent, _)) =>
        ...
        try
          if canceled() || !isCurrent() || synchronized {
              disposed ||
              generations.get(root) != Some(generation)
            }
          then None
          else
            val checked = backend.check(
              snapshot,
              () =>
            ...
            val selection = ApplicationManager.getApplication
              ...
              .selection
            val externalCurrent =
              checked.key.externalStamp == BendExternalInputs.stamp(
                checked.key.executable,
                checked.key.basePath
              )
            val published = synchronized {
              val facts = BendPublicationFacts(
                generation,
                generations.get(root),
                sourceCurrent,
                inputsCurrent,
  ...
  override def result(root: FileId): Option[BendCheckResult] =
    ...
    val graphChanged = if captured.exists(s => !graphCurrent(s)) then
      synchronized {
        results.get(root) match
          case Some(r)
              if r.fresh && captured
                .exists(snapshot => captures.get(root).exists(_ eq snapshot)) =>
    ...
    val externalCurrent = inspected.forall(result =>
    ...
    val (value, changed) = synchronized {
      results.get(root) match
        case Some(result)
            if result.fresh && inspected.contains(result) && !externalCurrent =>
