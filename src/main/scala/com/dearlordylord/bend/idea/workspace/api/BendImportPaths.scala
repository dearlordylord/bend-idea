...
object BendImportPaths:
  ...
  def target(sourcePath: String, packageCache: String, spelling: String): String =
  ...
  def directory(sourcePath: String, packageCache: String, fragment: String): String =
    ...
    if parentPath.endsWith("/") && normalized.matches("^0x[0-9a-f]+(?:/.*)?$") then
      normalize(packageCache + "/" + normalized)
