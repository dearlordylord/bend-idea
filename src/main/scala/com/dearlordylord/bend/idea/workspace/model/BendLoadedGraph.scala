...
final case class BendImport(spelling: String, alias: Option[String], offset: Int,
    syntaxProblem: Option[String] = None)

final case class BendSourceRecord(id: FileId, path: String, text: String, revision: Long,
    imports: List[BendImport])

final case class BendLoadedFile(source: BendSourceRecord, namespace: String)
final case class BendLoadedEdge(from: FileId, importLine: BendImport, requestedPath: String,
    namespace: String, target: Option[FileId])
...
enum BendGraphProblem:
  ...
  case NamespaceConflict(from: FileId, importLine: BendImport, target: FileId,
      previous: String, requested: String)
