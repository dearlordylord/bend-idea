...
final case class BendCheckSnapshot(
    root: FileId,
    path: String,
    text: String,
    sourceRevision: Long,
...
object BendAnalysisKey:
  ...
  def from(snapshot: BendCheckSnapshot): BendAnalysisKey =
    BendAnalysisKey(
      ...
      sourceDigest(snapshot.text),
      snapshot.toolchain.configurationRevision,
      snapshot.toolchain.executable,
      snapshot.inputFingerprint,
      BendCheckSnapshotProvenance.from(snapshot)
    )
...
/** Captured source identity and revision for editor projection and later
  * structured spans.
  */
final case class BendCheckedSource(
    id: FileId,
    path: String,
    text: String,
    revision: Long,
...
/** A rewritten import has no exact source span. All other text keeps its line.
  */
final case class BendRewrittenRange(
    originalStart: Int,
    originalEnd: Int,
    copiedStart: Int,
    copiedEnd: Int
...
/** A source line removed by the compiler loader. The line separator, when
  * present, remains in compiler text to preserve following line numbers, but
  * belongs to the synthetic line and is not an attributable code unit.
  */
final case class BendBlankedRange(
    line: Int,
    copiedStart: Int,
    copiedEnd: Int,
    compilerStart: Int,
...
/** Maps one immutable source version through both CLI materialization steps.
  ...
  */
final case class BendSourceMapping(
    source: FileId,
    revision: Long,
    originalPath: String,
    copiedPath: String,
    ...
    originalText: String,
    copiedText: String,
    rewrittenLines: Set[Int],
    rewrites: List[BendRewrittenRange],
    compilerText: String,
  ...
  private final case class EditRange(
      fromStart: Int,
      fromEnd: Int,
      toStart: Int,
      toEnd: Int
  ...
  private def inverse(edits: List[EditRange]): List[EditRange] = edits.map(
    edit => EditRange(edit.toStart, edit.toEnd, edit.fromStart, edit.fromEnd)
  )

  /** Validate that every code unit outside declared edits is actually
    * preserved.
    */
  private def preservedGaps(
      from: String,
      to: String,
  ...
  private def validBlanks: Boolean =
    val lineRangesValid = blankedImportRanges.forall { range =>
      ...
      (range.copiedStart == 0 || copiedText.charAt(
        range.copiedStart - 1
      ) == '\n') &&
      (range.copiedEnd == copiedText.length || copiedText.charAt(
        range.copiedEnd - 1
      ) == '\n') &&
  ...
  private def mapOffset(
      offset: Int,
      from: String,
      to: String,
      edits: List[EditRange],
      rejectEditStart: Boolean
  ): Option[Int] =
    if offset < 0 || offset > from.length then None
        ...
        (offset > edit.fromStart && offset < edit.fromEnd) ||
          (rejectEditStart && edit.fromStart < edit.fromEnd && offset == edit.fromStart)
      )
    then None
    else
      ...
      val boundary = edits
        .find(_.fromEnd == offset)
        .map(_.toEnd)
        .orElse(edits.find(_.fromStart == offset).map(_.toStart))
      boundary.orElse {
        val shift = edits
          .filter(_.fromEnd <= offset)
          .map(edit =>
            (edit.toEnd - edit.toStart) - (edit.fromEnd - edit.fromStart)
          )
          .sum
        val mapped = offset + shift
        Option.when(
          mapped >= 0 && mapped <= to.length &&
            (offset == from.length || mapped == to.length || from.charAt(
              offset
            ) == to.charAt(mapped))
        )(mapped)
      }

  private def mapRange(
      start: Int,
      end: Int,
      from: String,
      to: String,
  ...
  /** Exact compiler-input-to-copied offset; blanked line content and its
    * retained separator are synthetic. At an empty final blanked line, the
    * shared EOF boundary maps to the end of the removed source line.
    */
  def compilerToCopiedOffset(compilerOffset: Int): Option[Int] =
    if !validBlanks then None
      ...
      mapOffset(
        ...
        compilerText,
        copiedText,
        inverse(blankingEdits),
        rejectEditStart = true
      )

  /** Compatibility name for the original compiler-input-to-copied operation. */
  def copiedOffset(compilerOffset: Int): Option[Int] = compilerToCopiedOffset(
    compilerOffset
  )
  ...
  /** Compose compiler-input -> copied-source -> original-source for #41
    * adapters.
    */
  ...
  /** Exact structured diagnostic span mapping. Any rewritten or synthetic code
    * unit makes the full range unavailable, even when both endpoints happen to
    * map.
    */
...
final case class BendCheckResult(
    key: BendAnalysisKey,
  ...
  def status: String =
    val verdict = outcome match
      ...
      case BendCheckOutcome.Failed =>
        if completeness == BendCompleteness.Incomplete then "Check incomplete"
        else "Check failed"
    ...
    val relied = if reliance == BendReliance.UnsafeOrForeign then
      "; relies on unsafe or foreign code"
    else ""
