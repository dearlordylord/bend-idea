...
/** Stable keys emitted from file-local declaration syntax; query policy lives in symbols.index. */
object BendStubIndexKeys:
  ...
  def dottedComponents(name: String): List[String] =
    val suffixes = name.indices.filter(name(_) == '.').map(offset => name.substring(offset + 1))
    val segments = name.split("\\.").filter(_.nonEmpty).toList
    (suffixes.toList ++ segments).distinct.filter(component => component.nonEmpty && component != name)

/** Declaration facts stored in a stub are derived solely from that declaration's file. */
...
) extends IStubElementType[BendDeclarationStub, BendDeclaration](debugName, BendLanguage.instance):
  override def createPsi(stub: BendDeclarationStub): BendDeclaration = create(stub)

  override def createStub(psi: BendDeclaration,
      parent: StubElement[? <: PsiElement]): BendDeclarationStub =
    val name = Option(psi.getNameIdentifier).map(_.getText).getOrElse("")
    new BendDeclarationStub(parent, name, this)

  override def serialize(stub: BendDeclarationStub, data: StubOutputStream): Unit =
    data.writeName(stub.name)

  override def deserialize(data: StubInputStream,
      parent: StubElement[? <: PsiElement]): BendDeclarationStub =
...
/** Static element type fields are discovered by IntelliJ's stub holder extension. */
trait BendStubElementTypes

object BendStubElementTypes:
  @static val Definition: IStubElementType[BendDeclarationStub, BendDeclaration] =
    new BendDeclarationElementType("Definition", stub => new BendDefinition(stub))
  ...
  @static val Constructor: IStubElementType[BendDeclarationStub, BendDeclaration] =
    new BendDeclarationElementType("Constructor", stub => new BendConstructor(stub))

object BendFileElementType
    extends IStubFileElementType[PsiFileStub[?]]("BEND_FILE", BendLanguage.instance):
