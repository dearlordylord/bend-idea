...
sealed abstract class BendDeclaration(node: ASTNode) extends ASTWrapperPsiElement(node) with PsiNameIdentifierOwner:
  ...
  override def setName(newName: String): PsiElement =
    ...
    if original == null then throw new IncorrectOperationException("Declaration has no name")
    val replacement = com.intellij.psi.PsiFileFactory.getInstance(getProject)
      .createFileFromText("rename.bend", com.dearlordylord.bend.idea.syntax.BendLanguage.instance,
        s"def $newName():\n  0\n")
    val declaration = PsiTreeUtil.findChildOfType(replacement, classOf[BendDefinition])
    original.replace(declaration.getNameIdentifier)
    this

  def headerText: String = Option(PsiTreeUtil.getChildOfType(this, classOf[BendHeader])).map(_.getText.trim).getOrElse("")
  def sourceComments: String =
    ...
    if file == null then ""
      ...
      val indent = source.substring(lineStart, start).takeWhile(c => c == ' ' || c == '\t').length
      val lines = source.substring(0, start).split("\\r?\\n", -1).toList.dropRight(1).reverse
      lines.takeWhile { line =>
        val spaces = line.takeWhile(c => c == ' ' || c == '\t').length
        spaces == indent && line.drop(spaces).startsWith("#")
      }.reverse.map(_.trim.stripPrefix("#").stripPrefix(" ")).mkString("\n")
...
object BendSourceParameter:
  ...
  private def parseDelimited(source: String, at: Int, open: Char, close: Char): List[BendSourceParameter] =
    ...
    while p < source.length && closes.nonEmpty do
      ...
      if c == close && closes.size == 1 then
        ...
        c match
          ...
          case '[' => closes += ']'
          case '<' => closes += '>'
          case _ if closes.lastOption.contains(c) => closes.remove(closes.size - 1)
          case _ => ()
        current.append(c)
      p += 1
    ...
    parts.toList.flatMap { raw =>
      ...
      val name = beforeColon.reverse.takeWhile(c => c.isLetterOrDigit || c == '_' || c == '.').reverse
      Option.when(name.nonEmpty && name != "_")(BendSourceParameter(name, sourceText))
    }
