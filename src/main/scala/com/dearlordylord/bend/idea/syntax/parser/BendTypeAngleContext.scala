...
import com.dearlordylord.bend.idea.syntax.psi.{BendConstructor, BendDatatype, BendHeader}
import com.dearlordylord.bend.idea.syntax.lexer.{BendLexer, BendTokens}
import com.intellij.psi.PsiFile
import com.intellij.psi.util.PsiTreeUtil
import scala.collection.mutable

/** A deliberately narrow syntax judgment for editing type arguments in declaration headers. */
object BendTypeAngleContext:
  /** Paired type angles from the tolerant declaration and do-type surface. Other operators stay unmatched. */
    ...
    while lexer.getTokenType != null do
      ...
      if kind == BendTokens.DeclarationKeyword then
        ...
        if before.isEmpty || before.matches("@unsafe\\s+") then
          ...
          square = 0
          curly = 0
      if kind == BendTokens.Keyword && source.subSequence(start, end).toString == "do" then
        doType = true
      kind match
        case com.intellij.psi.TokenType.WHITE_SPACE if doType && opens.isEmpty &&
            source.subSequence(start, end).toString.exists(c => c == '\n' || c == '\r') => doType = false
        ...
        case BendTokens.LeftAngle if (header || doType || datatypeBody && curly > 0) &&
            typeNameBefore(source, start) => opens += start
        case BendTokens.RightAngle if opens.nonEmpty =>
          val open = opens.remove(opens.size - 1)
          pairs(open) = start
          pairs(start) = open
        case BendTokens.Separator if source.charAt(start) == ':' &&
  ...
  private def lineBeginning(source: CharSequence, offset: Int): Int =
    var at = offset - 1
    while at >= 0 && source.charAt(at) != '\n' && source.charAt(at) != '\r' do at -= 1
    at + 1

  private def typeNameBefore(source: CharSequence, offset: Int): Boolean =
    var at = offset - 1
    while at >= 0 && (source.charAt(at).isLetterOrDigit || source.charAt(at) == '_' || source.charAt(at) == '.') do
      at -= 1
  ...
  private def constructorFieldAt(file: PsiFile, offset: Int): Boolean =
    ...
    val constructor = PsiTreeUtil.getParentOfType(leaf, classOf[BendConstructor], false)
  ...
  private def doTypeAt(source: String, offset: Int): Boolean =
    ...
    while lexer.getTokenType != null && lexer.getTokenStart < offset do
      ...
      if kind == com.intellij.psi.TokenType.WHITE_SPACE && angleDepth == 0 &&
          source.substring(start, end).exists(c => c == '\n' || c == '\r') then active = false
      else if kind == BendTokens.Keyword && source.substring(start, end) == "do" then
        active = true
        angleDepth = 0
      else if active then
        kind match
          ...
          case BendTokens.Separator if source.charAt(start) == ':' && angleDepth == 0 => active = false
          case _ => ()
      lexer.advance()
    active
  ...
  def at(file: PsiFile, offset: Int): Boolean =
    ...
    val prefix = source.substring(if header == null then lineStart else header.getTextRange.getStartOffset, offset)
    val name = "([A-Za-z_][A-Za-z0-9_.]*)$".r.findFirstMatchIn(prefix).map(_.group(1))
    val datatype = header != null && PsiTreeUtil.getParentOfType(header, classOf[BendDatatype], false) != null
    val inDoType = header == null && doTypeAt(source, offset)
    name.exists(word => (header != null || inDoType || constructorFieldAt(file, offset)) &&
      (word.head.isUpper || datatype && prefix.matches("(?s).*\\btype\\s+" + java.util.regex.Pattern.quote(word))))
