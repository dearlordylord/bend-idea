...
/** Foldable ranges from current tolerant PSI and lexer tokens; no compiler or resolver judgment. */
object BendFoldingSurface:
  private final case class Line(from: Int, until: Int, indent: Int, content: String)

  def ranges(file: PsiFile): List[BendFoldRange] =
    val source = file.getText
    val result = mutable.LinkedHashSet.empty[BendFoldRange]
    val declarations = PsiTreeUtil.findChildrenOfType(file, classOf[BendDeclaration]).asScala
    declarations.foreach { declaration =>
      val header = PsiTreeUtil.getChildOfType(declaration, classOf[BendHeader])
      if header != null then add(source, result, header.getTextRange.getEndOffset,
        declaration.getTextRange.getEndOffset, BendFoldKind.Declaration)
    }
    ...
    while lexer.getTokenType != null do
      ...
      if kind == BendTokens.DeclarationKeyword && atColumnZero(source, offset) then delimiters.clear()
      ...
      if Set(BendTokens.RightParen, BendTokens.RightBracket, BendTokens.RightBrace).contains(kind) &&
          delimiters.lastOption.exists(_._2 == spelling.head) then
    ...
    for (keywordOffset, blockKind) <- blocks do
      lines.find(line => line.from <= keywordOffset && keywordOffset < line.until).foreach { line =>
  ...
  private def add(source: String, result: mutable.Set[BendFoldRange], from: Int,
      until: Int, kind: BendFoldKind): Unit =
    val end = math.min(until, source.length)
    var trimmed = end
    if kind != BendFoldKind.Expression then
      while trimmed > from && source.charAt(trimmed - 1).isWhitespace do trimmed -= 1
    if from >= 0 && trimmed > from && source.substring(from, trimmed).contains('\n') then
      result += BendFoldRange(from, trimmed, kind)
  ...
  private def blockColon(source: String, keywordOffset: Int, lineEnd: Int): Option[Int] =
    val lexer = new BendLexer()
    ...
    while lexer.getTokenType != null do
      ...
      kind match
        case BendTokens.LeftParen | BendTokens.LeftBracket | BendTokens.LeftBrace | BendTokens.LeftAngle => depth += 1
        case BendTokens.RightParen | BendTokens.RightBracket | BendTokens.RightBrace | BendTokens.RightAngle =>
          depth = math.max(0, depth - 1)
        case BendTokens.Separator if spelling == ":" && depth == 0 => colon = Some(lexer.getTokenEnd)
        case _ => ()
      lexer.advance()
    colon
