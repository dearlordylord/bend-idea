...
/** Tolerant declaration grammar. A column-zero declaration always ends the
  * preceding item, including an unfinished header or body. Expressions remain
...
final class BendSurfaceParser extends PsiParser:
  override def parse(root: IElementType, builder: PsiBuilder): ASTNode =
    ...
    while !builder.eof() do
      ...
      else if builder.getTokenType == BendTokens.NamespaceName then
        alias(builder)
      else builder.advanceLexer()
    file.done(root)
    builder.getTreeBuilt

  private def text(builder: PsiBuilder): String =
    Option(builder.getTokenText).getOrElse("")
  private def lineStart(builder: PsiBuilder): Int =
    val source = builder.getOriginalText
    val offset = builder.getCurrentOffset
    var p = offset - 1
    while p >= 0 && source.charAt(p) != '\n' && source.charAt(p) != '\r' do
      p -= 1
    p + 1
  ...
  private def indentation(builder: PsiBuilder): Int =
    ...
    var p = begin
    while p < builder.getCurrentOffset && (source.charAt(p) == ' ' || source
        .charAt(p) == '\t')
    do p += 1
  ...
  private def declarationStart(builder: PsiBuilder): Boolean =
    indentation(builder) == 0 &&
      ((builder.getTokenType == BendTokens.DeclarationKeyword && BendWords
        .declaration(text(builder))) ||
        (builder.getTokenType == BendTokens.Unsafe && builder.lookAhead(
          1
        ) == BendTokens.DeclarationKeyword))
  ...
  private def nameToken(builder: PsiBuilder): Boolean =
    Set(
      ...
      text(builder).matches("[A-Za-z_][A-Za-z0-9_.]*") && !text(builder)
        .endsWith(".") && !BendWords.reserved(text(builder))

  private def advanceReference(builder: PsiBuilder): Unit =
    val incompleteQualified =
      builder.getTokenType == com.intellij.psi.TokenType.BAD_CHARACTER &&
  ...
  private def declaration(builder: PsiBuilder): Unit =
    ...
    while !builder.eof() && !declarationStart(builder) && !colonFound do
      ...
      word match
        ...
        case _
            if Set(
              BendTokens.Operator,
              BendTokens.LeftAngle,
              BendTokens.RightAngle
            ).contains(builder.getTokenType) =>
          val source = builder.getOriginalText
          for i <- word.indices do
            val c = word.charAt(i)
            val before = builder.getCurrentOffset + i - 1
            if c == '<' && (stack.isEmpty || stack.lastOption.contains(
                ">"
              )) && before >= 0 &&
    ...
    while !builder.eof() && !declarationStart(builder) do
      if kind == "type" && colonFound && constructorStart(builder) then
        constructor(builder)
  ...
  private def constructor(builder: PsiBuilder): Unit =
    ...
    var depth = 0
    var started = false
    while !builder.eof() && !declarationStart(
        builder
      ) && (!started || depth > 0)
    do
      text(builder) match
