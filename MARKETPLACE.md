...
The [custom plugin repository feed](https://dearlordylord.github.io/bend-idea/updatePlugins.xml) is served by GitHub Pages from [`docs/updatePlugins.xml`](docs/updatePlugins.xml). The Git-connected Cloudflare Pages project `bend-idea-plugins` serves the same file at `https://bend-idea-plugins.pages.dev/updatePlugins.xml` and is configured for `idea.dearlordylord.com`; that domain remains pending until its authoritative DNS has a CNAME from `idea` to `bend-idea-plugins.pages.dev`. For each future pre-Marketplace release, verify that the signed ZIP is publicly downloadable first, then update the feed's `version`, `url`, and `idea-version` to match the ZIP's packaged `plugin.xml`. Publish the feed change only after the release asset is live. Cloudflare Pages deploys `docs/` from `master` automatically; the feed metadata is maintained manually, with no automatic release-to-feed workflow.
