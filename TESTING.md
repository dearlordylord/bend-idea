# Manual test guide: issues #34–#38
...
| [#38](https://github.com/dearlordylord/bend-idea/issues/38) | Inspect static dependencies | `battle/reducer/attack.bend` → `battle/attack.bend`; unresolved call: `bend-idea/imports/main.bend` | Put the caret on a declaration and use **Tools → Bend → Inspect Bend Dependencies** or right-click in the editor and use **Bend → Inspect Bend Dependencies**. Check incoming/outgoing calls and imports; use the unresolved fixture call to check that unknown dependencies are reported. Select a row to navigate. | Tools action works; verify the new editor context placement. |
