...
|-------------------------------------------------------------|---|---|---|---|
| [#33](https://github.com/dearlordylord/bend-idea/issues/33) | Run Bend explicitly | `bend-idea/run/main.bend`; larger real demo: `battle.bend` | Open `bend-idea/run/main.bend` and click the Run gutter icon beside `def main()`. Choose **Run** and inspect the Run console output; the created **Bend Run** configuration should already point at this file. Also confirm **Run → Edit Configurations** still lets you select a `.bend` root with its browse button. |
...
- [ ] Confirm `def main()` has a Run gutter icon that creates **Bend Run** for `bend-idea/run/main.bend`; other definitions and parameterized `main` declarations should not have one. Confirm the manual choices remain **Bend Run**, **Bend Build**, and **Bend Native**, with `.bend` file choosers for Run/Build roots.
